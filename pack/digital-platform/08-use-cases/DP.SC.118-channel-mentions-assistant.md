...
> **Авто-ответчик (Phase 2, будущее)** — бот отвечает в канал самостоятельно, пользователь модерирует постфактум. Выше риск, требует доверия к качеству ответов.
>
> Phase 1 → Phase 2: только после подтверждения качества черновиков (>80% используются без правок).

## Фазы реализации

| Фаза | Что | Когда |
|------|-----|-------|
| **Phase 1** | Мониторинг + черновик в личку (владелец) + уведомление (участники) | WP-5 текущий |
| **Phase 2** | Авто-ответ в канал + модерация постфактум | После валидации Phase 1 |
| **Phase 3** | Персональная база знаний для каждого участника (не только knowledge-mcp) | После Phase 2 |

## Ограничения Phase 1

- Только каналы/группы, где бот — админ с правом чтения сообщений
- Черновик ответа — только для владельцев/админов канала с подключённым IWE (T3+)
