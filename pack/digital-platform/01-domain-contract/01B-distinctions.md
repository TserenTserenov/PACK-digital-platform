...
### DP.D.034: Platform (L2) ≠ IWE Template (L3) ≠ Personal IWE (L4)
...
**Почему важно**: Разработка/интеграция без указания уровня → IntegrationGate нарушен. Правила, роли и процессы различаются по уровням. Одна и та же сущность (WakaTime) рекомендуется на L3, но настраивается на L4.

**Тест**: Затрагивает всех пользователей? → L2 (Platform). Только шаблон? → L3 (Template). Только конкретного пользователя? → L4 (Personal).

> Подробно: DP.EXOCORTEX.001 §11, DS-ecosystem-development/11-platform-levels.md
...
- [ ] Разрабатываете/интегрируете без указания уровня (Platform/Template/Personal)? (DP.D.034)
