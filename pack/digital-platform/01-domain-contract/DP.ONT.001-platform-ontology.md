...
| **ОРЗ-фрактал** | ORZ Fractal | Фрактальный паттерн работы: Открытие → Работа → Закрытие. Применяется на четырёх масштабах: Сессия, День, Неделя, Месяц (открытие формализовано на сессии и дне, закрытие — на всех четырёх) | U.Method | DP.M.003 |
...
| **Неудовлетворённость** | Dissatisfaction | Психическое состояние человека в ответ на проблему (задета потребность по спискам Маслоу/NVC). Сила (1-5) = мотивация решать; критерий ранжирования приоритетов. Всегда привязана к проблеме | U.Characteristic | DP.METHOD.053, DP.ROLE.012, DP.D.053 |
| **Эмоция / Чувство** | Emotion / Feeling | Психическая реакция, выражающая неудовлетворённость. Эмоция — состояние в моменте (минуты), чувство — на длинном масштабе (недели-годы). Не обязательно привязаны к неудовлетворённости | U.Characteristic | DP.METHOD.053 |
| **Состояние пилота** | Pilot State | Качественная характеристика положения пилота, ведущая выбор тактики разговора-распаковки: Хаос / Потолок / Поворот / Развитие. Фиксируется как state-card | U.Characteristic | DP.ROLE.012, DP.METHOD.053 |
| **ТОС-месяца** | Monthly Constraint | Главное ограничение месяца (Theory of Constraints), удерживающее остальные приоритеты; его снятие двигает результат максимально | U.Characteristic | DP.ROLE.012, DP.METHOD.053 |
| **Контекст приоритетов** | Priorities Context | Объект передачи Стратег → Плановик: приоритеты месяца + состояние пилота + системные уровни НЭП (не голый список приоритетов) | U.Episteme | DP.SC.051, DP.ROLE.012, DP.ROLE.066 |
...
| НЭП | Неудовлетворённость-Эмоция-Проблема (мнемоника слоёв разбора) | Dissatisfaction-Emotion-Problem | Pack |
| ТОС | Теория ограничений | Theory of Constraints | общее (Goldratt) |

---

## 8. Changelog

- 2026-06-05 — РП378 Ф1: добавлены в глоссарий понятия Стратега (Неудовлетворённость, Эмоция/Чувство, Состояние пилота, ТОС-месяца, Контекст приоритетов); аббревиатуры НЭП, ТОС. Закрыт онтологический долг DP.METHOD.053 §6.
