...
type: distinction
status: active
summary: "Данные пользователя растут с тиром платформы: T1 базовые знания (без персонализации) → T2 минимальная персонализация (профиль + история) → T3 цифровой двойник (ИИ знает контекст) → T4 личный контекст + ИИ-агенты (со-мыслитель) → T5 управление экосистемой"
created: 2026-02-18
edition: "2026-02"
trust:
  F: 3
...
|-----|----------|----------------|----------------|
| **T1** | **Старт** | Доступ к знаниям | Бесплатный бот + базовые знания платформы. ИИ — **справочник**: отвечает на вопросы по домену, но не знает пользователя |
...
| **T5** | **Администрирование** | Управление экосистемой | Исходный код всех систем + деплой + обновление standard/ для T2-T3. ИИ — **архитектор**: со-разработчик платформы |

### Секции "Мои метрики" по тирам

| Секция /mydata | T1 | T2 | T3 | T4 | T5 |
|----------------|----|----|----|----|-----|
| Профиль | Базовый (5 полей) | Расширенный (12 полей) | Полный ЦД | Полный ЦД | Полный ЦД |
| Активность | streak + days | + sessions | + sessions | + sessions | + sessions |
| Марафон | да | да | да | да | да |
| Лента | — | да | да | да | да |
| Консультации | — | count + helpful% | + DT-контекст | + DT-контекст | + DT-контекст |
| GitHub | — | заметки (OAuth) | заметки (OAuth) | заметки + git | заметки + git |
| AI "Почему?" | — | Haiku | Haiku + DT | Haiku + DT + personal | Haiku + DT + personal |
| AI "Как улучшить?" | — | Haiku | Haiku + DT | Haiku + DT | Haiku + DT |
| Мои знания | — | — | — | Pack + memory/ | Pack + memory/ + код |

### Рост данных по тирам

| Тир | Объём данных | Источники |
|-----|-------------|-----------|
| T1 Старт | ~15 полей | Профиль (онбординг) + марафон (бесплатный) |
...
| T5 Администрирование | ~100+ полей | = T4 + исходный код систем, PROCESSES.md, MAPSTRATEGIC всех репо |
...
| T5 | = T4 + доступ к исходникам и деплою всех систем |
