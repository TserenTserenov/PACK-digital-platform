...
| Force | Tension |
|-------|---------|
| Мощность IT-аналогии ↔ чистота доменной модели | Docker/кубер-метафоры помогают объяснить архитектуру новичкам и IT-специалистам, но импорт их терминов в имена доменных сущностей искажает домен |
| Общий междисциплинарный язык ↔ собственная терминология | Общие термины ускоряют коммуникацию, но размывают специфику системы; доменные термины точны, но требуют onboarding |
| Скорость onboarding через аналогию ↔ долгосрочная ясность архитектуры | Аналогия даёт быстрый старт, но если она становится identity, архитектурные границы начинают повторять docker-концепции вместо доменных |
...
|------|--------------------------|
| Переименование доменных сущностей в IT-термины | Вместо meta-языка аналогии используются имена вроде «Container Runtime», «Image Builder», «Pod Manager», нарушая S-13 и делая аналогию identity |
| Аналогия становится primary mental model | Практикующий забывает, что docker-термины — это meta-язык, и начинает проектировать доменные границы под docker-концепции |
| Предпочтение знакомых терминов незнакомым доменным | Знакомые слова «image», «pod», «volume» выигрывают у специфичных доменных имён в обсуждении, что постепенно переписывает глоссарий системы |
