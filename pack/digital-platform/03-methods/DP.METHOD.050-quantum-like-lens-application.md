...
| Force | Tension |
|-------|---------|
| Classical-first дисциплина (§1.2, исчерпать классический набор §2 из 6 пунктов) ↔ скорость распознавания скрытой запутанности | Обязательный проход через 6-пунктовый классический набор до входа в QL-lite снижает риск преждевременного «прыжка в QL» (§6), но задерживает признание симптома, когда практикующий уже подозревает order-effects или probe-coupling — метод жертвует скоростью диагностики ради дисциплины |
| Минимальный пол формализма (§1.3, order-table вместо Hilbert-space) ↔ полнота выражения incompatibility | Отказ от полного quantum state-space держит линзу применимой без физико-математической подготовки, но order-table/non-commutativity matrix может не выразить более тонкие формы несовместимости observables, которые уловил бы полный формализм |
| Все 5 предохранителей §4 обязательны одновременно ↔ скорость активации линзы при частичной ясности | «Если хотя бы один ответ отсутствует — линза не активирована» (§4) — жёсткий all-or-nothing гейт защищает от overclaim, но останавливает применение даже когда 4 из 5 предохранителей ясны, а пятый (критерий остановки) пока не сформулирован |
...
|------|--------------------------|
| Притяжение к нарративно яркому симптому в §2 | При сверке с таблицей §2 внимание тянется к самому «рассказываемому» симптому (probe-coupling / история про Гудхарта — §5 Пример 1) и недооценивает тише проявленные строки таблицы (cross-context evidence transfer, false composition), которые требуют больше работы для распознавания — практикующий останавливается на первом подтверждающем совпадении, не доходя до полной строки таблицы |
| Смещение фокуса при заполнении предохранителей §4 | Формулировка «самого слабого допустимого вывода» (предохранитель 3) и «действия» (предохранитель 4) ощущается содержательной работой и забирает внимание, а «критерий остановки» (предохранитель 5) — про будущее и труднее для обязательства сейчас; внимание смещается на первые два и не доходит до точной формулировки критерия остановки, хотя формально поле заполняется |
