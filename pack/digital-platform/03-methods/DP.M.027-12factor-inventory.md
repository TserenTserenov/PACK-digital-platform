...
| Force | Tension |
|-------|---------|
| Полнота реестра (охват ВСЕХ production units, шаг 1) ↔ трудоёмкость единовременного прохода | Включение ботов, воркеров, ML-сервисов, агентов, CF Workers и прочего в один реестр даёт возможность увидеть системный дефект (F2 ❌ у всех Python-сервисов), но каждый новый тип unit увеличивает объём заполнения матрицы (31 unit × 12 факторов в примере WP-307) |
| Статус `?` = риск = `⚠️` (Инварианты) ↔ скорость первого прохода | Приравнивание пустой клетки к риску вынуждает не откладывать неясные факторы «на потом», но замедляет первый проход — каждую пустую клетку нужно либо заполнить, либо осознанно принять как найденный риск |
...
|------|--------------------------|
| Заполнение по горизонтали вместо просмотра по вертикали | Внимание съезжает на заполнение строки за строкой (один unit — все 12 факторов подряд), хотя ценность метода — в просмотре по вертикали (шаг 3, «просмотреть каждый фактор по вертикали») — построчное заполнение организационно проще, но откладывает момент обнаружения системного дефекта до самого конца аудита |
| Недооценка пустой клетки как «нет данных» | Пустая клетка `?` систематически трактуется как «нет данных, разберёмся позже», а не как предписанный Инвариантами статус «риск, приравненный к ⚠️» — практикующий по умолчанию считает неизвестность нейтральной, а не сигналом |
