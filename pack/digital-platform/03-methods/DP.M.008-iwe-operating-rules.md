...
**Порог применения (пет-проекты и solo DS):** `/archgate` обязателен, если решение затрагивает **одновременно ≥2 слоя** (UI + state, state + storage, UI + API и т.п.). Изменение в одном слое (перенос компонента, рефактор одного модуля) — не требует архгейта. Решение, меняющее паттерн взаимодействия между слоями — требует всегда.
...
**Плановое ТО (weekly):** разбор при Week Close через вопросы #14 — что сломалось? какое правило не работало? Не event-driven («сломалось — чиним»), а calendar-driven («раз в неделю смотрим»).
...
**Реализация:** CLAUDE.md §2 Self-correction, skill `/verify`, protocol-close.md §7.
