...
| Force | Tension |
|-------|---------|
| Мультитаскинг (Правило 9): параллелизм 2-4 фокусных РП/день (мультипликатор 1.5–2.5x) ↔ coordination overhead | Метод сам формулирует порог («более 3-4 параллельных сессий — coordination overhead съедает выигрыш»), удерживая баланс явным числовым диапазоном, а не общим «больше параллельности — лучше» |
| ТО (Правило 12, «поддержание текущего состояния») ↔ Эволюция (Правило 14, «развитие системы») | Обе фиксируют изменение системы, но с противоположным вектором: ТО держит текущее состояние рабочим, Эволюция явно ищет, какие правила устарели и подлежат удалению; одна и та же ситуация («правило систематически обходится») для ТО — инцидент для журнала, для Эволюции — сигнал к удалению правила |
| Блокирующие гейты (Правило 8, «Стоп-краны») ↔ автономность исполнителя (Правило 11: «агент не запрашивает разрешение») | Метод одновременно требует обязательной проверки перед определёнными действиями (WP Gate, АрхГейт) и запрещает агенту спрашивать разрешение на каждый шаг («разрешение = нарушение паттерна P5»); граница между «это гейт, стой» и «это шаг, действуй автономно» держится конкретным списком §8, а не общим правилом |

## 2. Реестр правил

| # | Правило | Суть | Блокирующее? |
|---|---------|------|-------------|
| 1 | **ОРЗ** | Любая работа = Открытие → Работа → Закрытие. Четыре масштаба: сессия, день, неделя, месяц (решение автора 31.08.2026). Пропуск = потеря | Да |
...
**Масштабы:** Сессия (30 мин — 4 часа), День, Неделя, Месяц. Фрактальный паттерн: каждый масштаб содержит ОРЗ. Открытие формализовано на Сессии и Дне; неделя и месяц открываются через Strategy Session, закрытие есть на всех четырёх (Quick/Day/Week/Month Close), Month Close идёт после Week Close и до Strategy Session.
...
|------|--------------------------|
| Ежедневное закрытие вытесняет ретроспективу | Внимание смещается на выполнение отдельных протоколов (Правило 1 ОРЗ, используется ежедневно) и недооценивает Правило 14 Эволюцию — ежедневная ретроспектива (4 вопроса) легко превращается в формальность, когда практик фокусируется на «закрыть день», а не честно ответить «какое правило обходил» |
| Учёт по задачам вместо РП | Несмотря на явный тест Правила 3 («РП ≠ задача»), внимание практика уходит обратно к учёту по действиям («написать», «исследовать») вместо артефактов с критериями готовности — задача психологически привычнее и не требует формулировки критерия готовности до старта |
