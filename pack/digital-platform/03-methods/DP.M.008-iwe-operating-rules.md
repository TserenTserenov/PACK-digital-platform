...
type: method
status: active
summary: "Культура работы IWE: 14 элементов в трёх разрезах — протоколы (формализованные последовательности), навыки (нарабатываемые по ситуации), форматы (стандарты оформления). Реализация — в DS/FMT, инварианты — здесь"
created: 2026-03-24
trust:
  F: 4
  G: domain
...
| #12 Самоисправление и наблюдаемость | Навык | — |
...
| 12 | **Самоисправление и наблюдаемость** | Расхождение → немедленный фикс. Наблюдаемость: понимать как IWE пришёл к результату. Не спускать ошибки — «душнить» как профессор. Одна ошибка не повторяется дважды | Нет |
...
### Правило 12. Самоисправление и наблюдаемость
...
**Наблюдаемость (observability):** Нужно понимать, как IWE пришёл к любому результату. Каждый вывод, каждое решение, каждый артефакт должен быть прослеживаемым: откуда данные, какой логикой обработаны, почему именно этот результат. Если не можешь объяснить путь от входа к выходу — система непрозрачна, а значит неуправляема.

**Не спускать ошибки:** Подход — как у въедливого руководителя или профессора: каждое отклонение разбирается, каждая неточность фиксируется, каждое «и так сойдёт» ставится под вопрос. Если не быть «душнилой» — эффекта не будет. Ошибки, которые спустили, становятся нормой. Норма деградирует. Через месяц система выдаёт мусор, а все привыкли.
...
- Логировать решения агента (captures, self-correction в протоколах)
- Субагент-верификатор (Haiku R23) — альтернативная проверка с context isolation

**Реализация:** CLAUDE.md §2 Self-correction rule, skill `/verify`, protocol-close.md §7.
