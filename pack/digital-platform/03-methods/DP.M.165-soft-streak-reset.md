...
| Force | Tension |
|-------|---------|
| Удержание мотивации после пропуска ↔ «ставка» на непрерывность | Hard reset (30→0) даёт обвал мотивации и отток, soft reset сохраняет прогресс, но снижает ценность непрерывности — метод не решает этот спор a priori, а отправляет к A/B по retention D30 |
| Простота правила «обнулить» ↔ психологическая нелинейность streak | Обнуление — ноль параметров и одна строка кода, но потеря 30-дневного streak переживается как смерть прогресса; метод вводит decay (линейный −N/день или ×0.9 с полом 1) ценой параметров, которые нужно калибровать |
...
|------|--------------------------|
| Выбор формулы decay затмевает калибровку её параметров | Внимание на элегантности варианта (линейный A vs экспоненциальный B), а значения `decay_per_day`/`decay_factor` берутся по умолчанию — формула правильная, поведение произвольное |
| Механика алгоритма видна, порог внедрения — нет | Внимание фиксируется на самом снижении, а условие применимости (≥15% пользователей теряют streak ≥7 дней) остаётся непроверенным — soft reset внедряется «на всякий случай» или не внедряется при давно выполненном пороге |
