...
| Force | Tension |
|-------|---------|
| Полнота guard-покрытия (§1: guard перед каждой из 4 операций — CREATE/DROP/INSERT/ADD CONSTRAINT) ↔ читаемость и лаконичность миграции | Guard перед каждой операцией (`IF NOT EXISTS`, `IF EXISTS`, `ON CONFLICT DO NOTHING`) удлиняет скрипт и снижает его прямолинейность, но отсутствие хотя бы одного guard превращает «миграция выполнена один раз» в ложное чувство безопасности (§2 инвариант) |
| Платформо-независимость принципа (§1-§3) ↔ конкретность готового DDL-синтаксиса | Метод сознательно держит принцип платформо-агностичным, вынося конкретные шаблоны (PostgreSQL `DO $$...EXCEPTION`, MySQL `IF NOT EXISTS`) в DS-документацию проекта (§4) — это даёт переносимость между СУБД, но оставляет практикующего без готового кода: guard-синтаксис приходится подбирать заново на каждой платформе |
...
|------|--------------------------|
| Фиксация на happy path при написании | В момент написания миграции внимание застревает на вопросе «работает ли скрипт при первом запуске» и не доходит до сценария повторного запуска (retry в CI/CD, multi-instance deploy — оба перечислены в §0) — guard добавляется как формальность или пропускается для INSERT, потому что его отсутствие не ломает первый прогон |
| Смещение ревью к семантике схемы | Ревьюер, читающий DDL-миграцию, привычно смещает внимание к тому, ЧТО создаётся (имена таблиц, типы колонок), и недооценивает КАК это создаётся (наличие guard) — §3 существует чеклистом именно потому, что эта проверка систематически выпадает из обычного ревью корректности схемы |
