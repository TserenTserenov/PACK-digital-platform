...
| Force | Tension |
|-------|---------|
| Свобода обновления базы (drop-in, §3.1: «база не знает о расширениях») ↔ выразительность расширения | Drop-in позволяет обновлять платформу без риска сломать кастомизации, но ограничивает пользователя добавлением до/после шага (§3.1 ограничение: «нельзя заменить платформенный шаг»); replace/remove требуют overlay поверх (§4, правило выбора п.2) |
| Полный контроль overlay-патча (§3.2: add/replace/remove) ↔ хрупкость к переименованиям базы | §3.2 ограничение: «переименование шага в базе → патч не применяется» — явное указание операции и позиции даёт мощь, но привязывает патч к текущим именам шагов платформы |
| Простота редактирования 3-way merge («редактируй как обычно», §3.3) ↔ модульность | §4 отмечает «Модульность: ❌ всё в одном файле» для 3-way merge — пользователь получает привычный способ правки платформенного файла напрямую, но теряет свойство «файл = расширение», из-за чего кастомизации нельзя переиспользовать или переносить по отдельности |
...
_Куда систематически съезжает внимание практикующего. Статус карточки — `draft` (epistemic_stage: `hypothesis`): пометка `tentative` по прецеденту SA.METHOD.001 (WP-448 Ф12)._

| Bias | Direction of distortion |
|------|--------------------------|
| _(tentative)_ Мощные паттерны выбираются по умолчанию | Практик смещается к overlay или 3-way merge (кажутся мощнее, §3.2/3.3), недооценивая, что drop-in по правилу выбора (§4: «по умолчанию — drop-in, покрывает 80% сценариев») уже достаточен для большинства случаев добавления шага/секции/проверки |
| _(tentative)_ Обновление важнее обнаружения противоречий | При проектировании протокола обновления (§5.1) внимание смещается на финальный шаг «Применить обновление» и недооценивает шаги 2–3 (обнаружение противоречий: ссылки на удалённые шаги, неприменимые патчи) — предупреждения (WARN) легко пропустить, если обновление автоматизировано и не требует явного подтверждения |
