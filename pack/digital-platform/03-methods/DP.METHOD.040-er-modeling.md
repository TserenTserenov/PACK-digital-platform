...
4. **«Одна ER на всё».** Попытка уместить 9 БД на одну ER. → Одна ER на BC (bounded context), обзорная картинка §4.0 показывает кластеры, не детали.
...
|----------|---------|----------------|
| `DP.ARCH.004 §4.0` | Обзор | 9 БД как кластеры + связи между ними (по одному узлу на БД, без таблиц) |
