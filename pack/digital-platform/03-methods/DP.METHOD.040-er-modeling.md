...
| Force | Tension |
|-------|---------|
| Строгость теста «можно ли показать пальцем» (§1.2, §1.4) ↔ скорость первого наброска ER-диаграммы при проектировании новой БД | Проверка каждой сущности на «объект физ.мира vs техническая абстракция» замедляет черновик, но её пропуск ведёт к антипаттернам §5.1-§5.2 на более поздней и дорогой стадии |
| Точность нотации Chen (различает атрибуты сущности/связи, §2) ↔ узнаваемость Crow's foot, более распространённой в индустрии | Метод сознательно жертвует привычной нотацией ради того, что Chen напрямую поддерживает правила трансформации §3 |
| Гранулярность «одна ER на bounded context» (§4 чек-лист, §6) ↔ желание видеть все 12 БД на одной обзорной картинке (§4.0) | Обзор §4.0 показывает кластеры без деталей именно потому, что детальная ER на 12 БД одновременно нарушила бы §1 читаемость и тест «можно показать пальцем» |
...
_Куда систематически съезжает внимание практикующего. Отличается от §5 «Антипаттерны»: там — итоговая ошибка на диаграмме, здесь — куда до того съезжает фокус внимания при работе._

| Bias | Direction of distortion |
|------|--------------------------|
| Notation Chen воспринимается как формальность, а не средство различения | При рисовании §2 практикующий видит выбор Chen как произвольную стилистическую конвенцию и незаметно соскальзывает на привычную Crow's foot — теряя различение атрибутов сущности/связи, которое как раз и нужно для трансформации §3 |
| §4 Чек-лист проходится по памяти, не построчно | При ревизии (§7 п.2) внимание смещается с построчной сверки чек-листа на общее «выглядит нормально» — пункты «каждая сущность имеет хотя бы одну связь» и «кардинальности проставлены» легко визуально пропустить на сложной диаграмме |
