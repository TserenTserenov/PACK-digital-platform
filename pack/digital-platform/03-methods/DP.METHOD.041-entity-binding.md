...
| Force | Tension |
|-------|---------|
| Единый источник правды (§1: запрет backlink `physical_realization` в Pack-сущности) ↔ удобство обратной навигации | Только §10 хранит связь, а обратный путь от Pack-сущности к реализации — это `grep`-поиск, а не прямая ссылка; это защищает от drift, но каждая навигация требует лишнего действия вместо клика по полю |
| Полнота drift-детектора (§3: 4 инварианта — непустая Pack-сущность, физ.объект есть в §10, DP.D/CONCEPT с физ.реализацией отражён, путь DS-реализации существует) ↔ стоимость проверки вручную | Формализация в скрипт `check-entity-binding.py` отложена в PMB WP-228 — до реализации все 4 проверки идут глазами, и чем больше §10, тем дороже полный обход |
| Обязательность sync при ARCH major bump (§4.1: grep+sync одним коммитом, owner = автор bump'а) ↔ риск search-replace без разбора (§5.6) | Механическая замена формулировки по всему репо ломает архивные и исторические контексты — метод требует whitelist и ручную проверку каждой замены, а не автоматический sweep |
...
|------|--------------------------|
| Code-first при новой таблице | При появлении новой таблицы (§4 «Новая таблица в БД») внимание смещается сразу к написанию миграции и query-файла — хотя правило одновременности (§1, §4) требует, чтобы §10 и Pack-сущность правились в том же коммите, DS-реализация подсознательно воспринимается как «настоящая» работа, а запись в §10 — как бюрократия, дописываемая потом |
| Соблазн backlink ради удобства | Под давлением частых навигаций внимание съезжает к добавлению `physical_realization: [...]` во frontmatter Pack-сущности — несмотря на явный запрет §5.1, локальное удобство («один клик вместо grep») перевешивает осознание риска рассинхронизации при переносе таблицы |
