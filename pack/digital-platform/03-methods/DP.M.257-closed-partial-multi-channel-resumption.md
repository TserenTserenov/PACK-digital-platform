...
| Force | Tension |
|-------|---------|
| Закрытие РП ↔ сохранение контекста | Закрыть как done — чисто для бэклога, но теряется обоснование и спека для v2; closed-partial сохраняет контекст, но держит «открытую» запись, которую хочется закрыть |
| Надёжность resumption ↔ когнитивная нагрузка | Пять каналов дают robustness при потере любых 2-3, но пилот воспринимает их как шум и склонен сократить до удобных |
| Минимальный процесс ↔ защита от забвения | Меньше каналов проще поддерживать, но любые 1-2 канала легко потерять (reminder пропущен, backlog забыт, inline comment устарел) |
...
|------|--------------------------|
| Предпочтение `done` для «чистоты» бэклога | Пилот/агент склонны закрыть РП как done, недооценивая риск потери обоснования и спеки для v2 |
| «5 каналов — это overkill» | Склонность сократить до 1-2 удобных каналов, что ломает robustness при их потере (reminder, catalog, inline comment) |
| Игнорирование threshold как «неопределённость» | Threshold откладывается как «когда-нибудь», а не превращается в explicit trigger с owner'ом, и РП забывается |
