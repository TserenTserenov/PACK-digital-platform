...
| Force | Tension |
|-------|---------|
| Строгость обязательного ребра к FPF-корню (принцип 1, §1) ↔ скорость создания нового Pack-документа | Каждый документ должен иметь `specializes`/`related` к U.*; для нового/неочевидного prefix это требует пройти 6 вопросов шага 3 §3, вместо того чтобы просто добавить документ и продолжить работу |
| Fallback U.Episteme (принцип 3) при неочевидности ↔ точность онтологической классификации | Fallback предотвращает orphan-понятия (граф не рвётся), но создаёт риск, что U.Episteme станет «мусорной корзиной» для всех неоднозначных случаев, если обязательный при fallback issue [ontology-mapping] не заводится или не разбирается |
| `specializes` (сильная связь) ↔ `related` (слабая связь) | Таблица §4 показывает конкретный кейс (DP.FM.010 → `related → U.Work`, не `specializes`) — метод требует осознанного выбора типа ребра, но разница между «косвенно относится» и «специализирует» не формализована количественно и остаётся эвристическим суждением практикующего |
...
|------|--------------------------|
| Fallback U.Episteme выбирается первым, а не последним | При столкновении с новым prefix внимание съезжает сразу к fallback U.Episteme (принцип 3) как к «безопасному быстрому выходу», минуя обязательные 6 вопросов шага 3 §3 — U.Episteme кажется нейтральным умолчанием, хотя метод предписывает использовать его именно как последний вариант, с обязательным issue [ontology-mapping] |
| Переоценка `specializes` относительно `related` | Выбор между `specializes` и `related` (таблица §4) систематически смещается в сторону `specializes` — она выглядит как «более полноценная» связь, интуитивно предпочтительнее слабой, хотя пример DP.FM.010 в §4 явно показывает случай, где корректна именно слабая связь `related`; переоценка строгости связи искажает граф так же, как и его недостаточная связность |
