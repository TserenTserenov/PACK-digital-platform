...
|------|--------------------------|
| Внимание фиксируется на первом условии | `status != completed` очевиден и покрывает большинство; при переносе фильтра в новый контекст (другой дашборд, другой тип артефакта) второй дизъюнкт — completed-но-зависшее — забывается, и метод тихо вырождается в одноусловный |
| TTL выбирается один раз | Календарная специфика (выходные, отпуск) делает фиксированный порог ложно-тревожным; внимание при шуме тянется отключить проверку целиком, а не перекалибровать TTL — гигиена теряется вместе с шумом |
