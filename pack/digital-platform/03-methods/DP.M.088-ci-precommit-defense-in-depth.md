...
| Force | Tension |
|-------|---------|
| Скорость локального feedback ↔ полнота покрытия путей коммита | Pre-commit hook даёт мгновенный fail до попадания в репо, но слеп к `--no-verify`, headless git и peer-agent workflow; GitHub Action покрывает все пути, но feedback приходит только после push/PR — метод платит поддержкой двух барьеров за оба свойства сразу |
| Единый источник инвариантов ↔ стоимость раскатки копий | pack-lint.sh один на всех, но Action-файл копипастится в каждый Pack-репо с параметрами репо; при изменении lint-скрипта нужно обновлять workflow во всех репо — метод принимает риск дрейфа копий ради серверного enforcement там, где нет централизованного runner'а |
...
|------|--------------------------|
| Зелёный локальный hook затмевает агентский путь коммита | Практик видит pre-commit fail на своей машине и считает инвариант защищённым; серверный Action воспринимается как «дубль для параноиков», а не как единственный барьер для `--no-verify`, headless-диспетчера и peer-agent'ов — то есть для именно тех путей, которые сама карточка называет мотивацией второго слоя |
| Добавление нового инварианта затмевает его раскатку | Внимание уходит в корректность нового R{N}-критерия в pack-lint.sh, а обновление workflow-файлов во всех Pack-репо откладывается: инвариант enforced локально у автора и нигде больше — defense-in-depth существует на одной машине |
