...
| Force | Tension |
|-------|---------|
| Защита от ложного пропуска ↔ риск ложной блокировки | OR двух независимых сигналов (MANUAL_CLOSE / git log vs state-file scheduler'а) гарантирует, что закрытие любым актором засчитывается, но каждый сигнал — точка отказа: сломанный state-file при выполненном ритуале заблокирует переход |
| Простота одной проверки ↔ покрытие обоих акторов | Один сигнал проще, но ручное и автономное закрытие пишут разные артефакты (переменная окружения/коммит vs файловый флаг); метод платит сложностью OR-логики за то, что ни человек, ни scheduler не получают ложный пропуск |
...
|------|--------------------------|
| Настройка блокировки важнее гарантии сигнала | Внимание практика уходит в логику gate (что и как блокировать), а связка с AS.M.030 — гарантия, что автономный сигнал вообще появится, — остаётся за кадром; gate без safety-net превращается в постоянный блокер ритуала |
| Проверка одного «удобного» сигнала вместо двух независимых | При переносе паттерна внимание съезжает к сигналу, который проще проверить (state-file), и забывается, что ручное закрытие пишет другой артефакт (MANUAL_CLOSE / git log) — OR-логика тихо деградирует до single-signal с ложными пропусками |
