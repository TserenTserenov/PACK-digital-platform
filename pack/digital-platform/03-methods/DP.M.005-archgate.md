...
type: method
status: active
summary: "Блокирующая оценка архитектурного решения: 5 характеристик (ЭМОГС) + SOTA-сверка. Без прохождения — решение не принимается"
created: 2026-02-13
trust:
  F: 4
  G: domain
...
Блокирующий метод оценки архитектурного решения перед его принятием. Включает проверку по 5 архитектурным характеристикам платформы (DP.ARCH.001) и сверку с SOTA-практиками. Без прохождения АрхГейта архитектурное решение не принимается.
...
| **Обработка** | Оценка по 5 характеристикам → SOTA-сверка → вывод |
| **Выходы** | Таблица оценки + вывод (что слабо, что менять) или блокировка |

## 3. 5 характеристик (ЭМОГС)
...
После оценки по 5 характеристикам — приоритетная тройка SOTA:
