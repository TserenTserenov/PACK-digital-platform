...
| Force | Tension |
|-------|---------|
| Семантическая полнота словаря ↔ скорость написания контента | Gate требует Pack-запись для каждого вводимого понятия и останавливает vocabulary drift, но каждое новое понятие обязывает к формализации в ontology.md — иначе gap надо явно исключать из scope |
| Полнота проверки ↔ стоимость CI-прогона | Полный режим проверяет все понятия во всех guide-файлах и ловит накопленный drift, но дорог на большом репо; diff-режим дёшев, но видит только понятия из файлов текущего PR — метод держит оба формата запуска |
...
|------|--------------------------|
| Зелёный файловый CI принимается за семантическое покрытие | Внимание практика успокаивается валидацией frontmatter и структуры файлов (уровень DP.M.088) и не замечает понятий, введённых в контент без Pack-записи, — vocabulary drift накапливается именно потому, что файловые проверки проходят |
| Gap закрывается исключением вместо формализации | При срабатывании gate внимание съезжает к ветке «явно исключить из scope» как самому быстрому способу разблокировать PR, хотя шаг 5 ставит добавление Pack-записи первым вариантом, а исключение — осознанным fallback |
