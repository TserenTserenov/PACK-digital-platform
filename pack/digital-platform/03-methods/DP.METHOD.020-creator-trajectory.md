...
Ступени **не назначает человек** — они отражают реальное поведение, а не самооценку. Это утверждение — про официальное платформенное значение (когда есть поведенческий след). Там, где следа нет (нет платформы, нет подключения), самоотчёт ступени легитимен как приближение; не путать с самоназначением степени квалификации ниже — см. [DP.D.252](../02-domain-entities/DP.D.252-stage-vs-qualification-degree.md).
...
| Force | Tension |
|-------|---------|
| Три независимых оси (роль/ступень/степень) ↔ простота коммуникации с человеком | Метод настаивает, что это три разных понятия, отвечающих на разные вопросы, но в разговоре с пользователем («хочу лучше исследовать») естественно смешать их в одну шкалу «насколько ты хорош» — различение методологически точное, но требует дисциплины удерживать три оси одновременно |
| Автоматический расчёт ступени по поведению ↔ легитимность самоотчёта при отсутствии платформы | Метод признаёт оба источника валидными («там, где следа нет, самоотчёт легитимен как приближение»), но не даёт правила перехода между ними — когда часть истории поведенческая, а часть самооценка, как их сочетать в одной траектории, не формализовано |
| Фокус на нижней роли ↔ желание человека работать с другой ролью | Метод явно признаёт конфликт («это допустимо, но важно объяснить, почему нижняя важна») — предписывает Навигатору лавировать между методологической правотой и согласием человека, не давая критерия, когда уступить желанию, а когда настоять |
...
|------|--------------------------|
| Сжатие пяти ступеней в одно суммарное впечатление | Внимание съезжает на числовую ступень как итоговую оценку человека — метод описывает ступень как «насколько хорошо исполняешь роль» (одну из пяти), но практика легко сворачивает пять независимых ступеней в одно впечатление «этот человек — Систематический», теряя различение по ролям |
| Переоценка роли «Ученик» как универсального диагноза | В приведённом примере Навигатор находит причину в ритме/дисциплине Ученика (раздел «Как Навигатор использует это») — внимание практика тяготеет к этому готовому объяснению вместо проверки, действительно ли слабое звено — именно нижняя роль в конкретном случае |
