...
| Force | Tension |
|-------|---------|
| Дешёвый early-reject (Phase 1, <1ч, дефолтные параметры) ↔ риск ложного отказа от рабочей гипотезы | Phase 1 намеренно не оптимизирует веса/prompt-template ради скорости, но именно из-за дефолтных параметров baseline может показать сигнал ниже threshold для гипотезы, которая при настройке (как r=0.612→0.761 в WP-316) оказалась бы рабочей — экономия бюджета создаёт риск отклонить недооценённую по дефолту гипотезу |
| Экономия 2-3 рабочих дней (early-reject) ↔ инвестиция в parameterized tuning (Phase 2) | Если Phase 1 не пройдёт reject-threshold, вложение в Phase 2 (настройка весов, prompt-template, sample size) не делается вовсе — метод разменивает потенциальную упущенную выгоду от недоинвестированной гипотезы на гарантированную экономию бюджета на заведомо мёртвых гипотезах |
...
|------|--------------------------|
| Baseline «дефолт» подгоняется под ожидаемый результат | При выборе «дефолтных параметров» для Phase 1 внимание тянется к параметрам, о которых уже что-то известно из предыдущего опыта, а не к настоящим library-defaults — это незаметно завышает сигнал baseline и подрывает саму цель Phase 1: дешёвый, непредвзятый early-reject |
| Прохождение threshold в Phase 1 принимается за подтверждение гипотезы | Как только baseline (r=0.612 в прецеденте) переваливает reject-threshold, внимание смещается к «гипотеза жива», и Phase 2 воспринимается как техническая настройка параметров под заведомо верный результат, а не как самостоятельная проверка «confirm at peak» — решение после Phase 2 незаметно предвосхищается уже на выходе из Phase 1 |
