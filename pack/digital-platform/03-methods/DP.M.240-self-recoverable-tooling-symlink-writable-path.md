...
|------|--------------------------|
| «Работает сейчас» вместо «работает из launchd» | Проверка выполняется в интерактивном шелле; сверка `launchctl getenv PATH` пропускается, и симптом «в терминале работает, в фоне нет» всплывает через недели, когда контекст установки уже забыт |
| Копия ставится и забывается | После правки инструмента в репо внимание не возвращается к установленной копии — drift SoT↔PATH накапливается молча; симлинк решает это автоматически, но выбирается копия «на всякий случай», без triggering-условия для повторного deploy |
