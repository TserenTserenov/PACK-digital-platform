...
| Force | Tension |
|-------|---------|
| Единый конвейер онбординга ↔ разные возможности пилота (Sovereign vs Managed) | Метод держит один pipeline Этапов 0-4 с принципом ВДВ, но платит ветвлением: gate перед Этапом 0 пропускает Этапы 1-2 для Managed, таблицы артефактов дублируют колонки Sovereign/Managed, а миграция T3a → T3b остаётся отдельным кейсом (WP-309 Ф9) |
| Автоматизация доставки (webhook → reindex ≤30 сек, stage_transition_listener) ↔ ручные действия пилота | Установка GitHub App на Этапе 2 — неустранимое ручное трение (OAuth, 3-5 шагов, polling до подтверждения); метод минимизирует его предзаполненным URL и пошаговой инструкцией, но gate «пилот нажал Install» остаётся человеческим и блокирует весь downstream |
...
|------|--------------------------|
| Штатный путь с готовым RCS затмевает fallback-путь нового пилота | Практик проектирует и проверяет render под полный RCS-профиль (ступень, bottleneck, доменная вставка), а сценарий «RCS отсутствует → fallback stage-1-random → только profile.md с одним вопросом» остаётся недопроверенным — хотя именно через него проходит каждый первичный онбординг |
| Первичный онбординг затмевает автопересборку при смене ступени | Внимание сосредоточено на первом проходе Этапов 0-4, а второй жизненный цикл Этапа 3 (stage_transition_listener → пересборка, архивация weekly/daily в history/) воспринимается как «тот же этап» — и остаётся STUB (WP-309 Ф7), хотя пересборок в жизни пилота больше, чем первичных запусков |
