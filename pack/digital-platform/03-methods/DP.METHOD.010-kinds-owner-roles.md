...
| Force | Tension |
|-------|---------|
| Строгость онтологического порядка (Kind→Owner→BC→лексика, §3) ↔ скорость создания сущности | Метод требует пройти 4 шага прежде чем создавать frontmatter-связи, но для очевидного случая (явный Kind, единственный Owner) это выглядит избыточной церемонией — быстрого пути для простых случаев метод не даёт, только полный проход |
| «Один Kind — один Owner» (жёсткое правило, §3 Шаг 2) ↔ реальная множественность претендентов | В примерах (табл. §3 Шаг 2) роль Owner не всегда однозначна — двойное указание в одной ячейке («R6 Кодировщик / Pack-автор») уже намекает на размытость, а правило требует единственности; конфликт между практикой (несколько кандидатов) и формой (один Owner) метод не разрешает явно |
| PROPOSE-only при найденном дубле (§3 Шаг 4) ↔ скорость bulk-обработки | При массовой миграции/экстракции (десятки кандидатов) остановка на каждом потенциальном дубле для ручного одобрения создаёт узкое место — метод оптимизирован для единичного случая, не для батча |
...
|------|--------------------------|
| Identity-критерий проверяется один раз при создании, не пересматривается при правке | Внимание съезжает на момент создания сущности (Шаг 1 пройден, поле в frontmatter проставлено) — при последующем изменении `definition` (§4, п.4) практик проверяет наличие `supersedes:`, но не возвращается к вопросу «это всё ещё тот же identity-критерий, что и был?»; это отдельный от FM «начать с лексики» дрейф — там ошибка в МОМЕНТ создания при сравнении с существующим, здесь — отсутствие повторной проверки СВОЕЙ ЖЕ сущности со временем |
| Недооценка Bounded Context против Kind/Owner | Метод даёт подробный тест для Kind (Шаг 1) и Owner (Шаг 2), но для BC (Шаг 3) — только «в каком Pack живёт», без явного теста; внимание практика фокусируется на первых двух шагах, третий проходится по инерции |
