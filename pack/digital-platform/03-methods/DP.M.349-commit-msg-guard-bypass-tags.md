...
| Force | Tension |
|-------|---------|
| Строгость защиты ↔ легитимный escape hatch | Guard без bypass надёжнее блокирует ошибку, но делает невозможным осознанное исключение; bypass-токен возвращает гибкость ценой риска злоупотребления |
| Ранняя проверка (pre-commit) ↔ доступность bypass-токена | pre-commit срабатывает раньше и быстрее прерывает, но токен ещё не написан в этот момент — opt-out физически невозможен на этой стадии |
| Простой diff-filter=A ↔ полное покрытие векторов | Фильтр только по Added короче и привычнее, но пропускает Renamed — тот же результат (файл в защищённой директории) достигается другим git-действием |
| Свобода использования bypass ↔ аудируемость обхода | Токен должен быть доступен разработчику без лишних вопросов, но частое использование должно быть заметно (>2/неделю — сигнал), иначе guard превращается в фикцию |
...
|------|--------------------------|
| «pre-commit — стандартное место для guard'ов, ставим туда» | Практикующий выбирает привычную точку хука по умолчанию, не проверяя, доступно ли на этой стадии сообщение коммита |
| «`--diff-filter=A` — этого достаточно, файл же добавляется» | Внимание фиксируется на прямом сценарии (новый файл), а `git mv` в защищённую директорию как вектор остаётся невидимым |
| «Bypass-токен есть — использую при любом неудобстве» | Лёгкость обхода смещает поведение от «редкое осознанное исключение» к «стандартный способ обойти проверку», аудит частоты не проводится по умолчанию |
| «Guard работает — тестировать бэкдоры не нужно» | Проверяется только основной сценарий блокировки, а не то, что каждый защищённый вектор (Added и Renamed) действительно ловится |
