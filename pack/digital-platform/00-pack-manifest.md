...
| DP.D.290 | Измерительный вопрос ≠ Интервенционный вопрос | D | Вопрос, питающий gate/метрику, формулируется без пресуппозиций (искажают самоотчёт); интервенционный вопрос (рефлексия собеседника) может использовать пресуппозицию, если её можно отвергнуть без цены. | active |
...
| DP.D.292 | Подписка (Pack-каталог) ≠ Поделиться (обычный репозиторий) | D | Формальная подписка через каталог применима только к Pack/DPF; обычные репозитории передаются вручную, без каталожного механизма. | active |
| DP.D.293 | Self-referential verification ≠ third-party verification | D | Сравнение двух подконтрольных себе значений ≠ сверка с независимым источником; третий член «семьи false-green различений». | active |
| DP.D.294 | Карантин недоверенного человеческого события ≠ обнаружение подделки агентского события | D | Защита от недоверенного, но подлинного источника (карантин) — не то же самое, что защита от подделки авторства (нужна подпись, не сверка по строке). | active |
...
| DP.FM.389 | Один коммит-развязка может откатывать одновременно markdown-карточки и исполняемый код | FM | — | active |
| DP.FM.390 | Блайнд git stash pop орфанного автостэша без построчной проверки deleted: | FM | — | active |
| DP.FM.391 | SKIP-ветка с exit 0 в обход общего finalize-пути проглатывает ранее записанный провал | FM | — | active |
| DP.FM.392 | Naive text read of serializer-quoted value accumulates escaping on every round-trip | FM | — | draft |
| DP.FM.393 | Runtime-зеркало неотличимо от исходного репозитория: удаляется чистками, гонка тиков без лока оставляет его битым | FM | — | draft |
| DP.FM.394 | Сквозной побочный эффект реализован только в одном из нескольких входных путей операции | FM | — | draft |
...
| DP.M.420 | Файл состояния инцидента: один алерт, окно повторов, явное сообщение о восстановлении | M | — | draft |
| DP.M.421 | Тест на поведенческое отличие именованных режимов: сотри имя ветки — упадёт ли тест? | M | — | draft |
| DP.M.422 | Read-model расширение с одного значения на ось на множественные права — таблица-сестра | M | — | active |
| DP.M.423 | Внутренний trust boundary для получателя аудита через компенсирующий контроль | M | — | active |
| DP.M.424 | Переиспользовать уже задеплоенный IdP вместо временной локальной заглушки | M | — | active |
| DP.M.425 | Non-fatal синхронизация репозитория-зависимости перед чтением опциональных данных | M | — | active |
...
> *Auto-generated by `generate-map.py` on 2026-08-09*
