...
| DP.ARCH.002 | Тиры обслуживания платформы | ARCH | 5-тирная модель обслуживания: T1 Старт → T2 Изучение → T3 Персонализация → T4 Созидание (IWE) → T5 Администрирование. Название тира = действие пользователя. Каждый тир определяет знания, персональные данные, роль ИИ и доступные действия | draft |
...
| DP.D.028 | User Data Tiers — тирование данных пользователя | D | Объём и тип данных пользователя растёт с каждым тиром: T1 базовый профиль → T2 полный профиль + история → T3 цифровой двойник → T4 личная база знаний | active |
...
> *Auto-generated by `generate-map.py` on 2026-02-18*
