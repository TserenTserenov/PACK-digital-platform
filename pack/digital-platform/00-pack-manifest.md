...
| DP.SC.062 | Публикация курируемого манифеста в исследовательский репозиторий iwesys | SC | Ретранслятор канона (DP.ROLE.081) переводит и публикует ЯВНО ПЕРЕЧИСЛЕННЫЙ манифест файлов из рабочего governance-репозитория (не весь repo/docs) в целевой публичный репозиторий iwesys — третий режим существующей роли, для потребителей без диспозиции 'вся документация репозитория публична' (WP-493 Лаборатория, будущий РП495). | draft |
...
> *Auto-generated by `generate-map.py` on 2026-07-20*
