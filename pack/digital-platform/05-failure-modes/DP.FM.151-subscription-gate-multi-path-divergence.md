...
related:
  see_also: [DP.FM.148, DP.FM.149, DP.FM.137, DP.FM.142, DP.FM.394, DP.FM.396]
...
- **Смежный вариант — guard одной точки вызова, соседи без защиты.** Тот же общий вопрос («формула проверки продублирована/пропущена в N местах») встречается и на уровне guard'а/детектора: он защищает ровно ту точку вызова, где нашли проблему, а соседние точки вызова того же примитива остаются без аналогичной защиты вовсе (не расходящаяся формула, как выше, а отсутствующая копия защиты). Тест: «этот guard стоит во всех местах, где вызывается защищаемый примитив, или только там, где баг впервые заметили?» Источник варианта: `feed:git-diff 2026-09-09`; отчёт `DS-my-strategy/inbox/extraction-reports/2026-09-11-inbox-check-2.md`, кандидат #3; дозапись R15 — 11.09 (РП-569 Ф2). Первичный коммит указан в отчёте; его повторная проверка этим уточнением не заявляется.
- Семейство асимметрии защиты по входным путям: DP.FM.137 — подавление алертов; DP.FM.142 — повтор; DP.FM.394 — журналирование; DP.FM.396 — обход существующей дедупликации. Эти связи не объявляют карточки дублями.
