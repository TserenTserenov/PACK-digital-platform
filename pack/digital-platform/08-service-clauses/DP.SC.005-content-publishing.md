...
|---|-----|-----|--------|----------|
| 0 | Собрать контент-бриф (сегмент/тон из карты ЦА + мем P-NNN/M-NNN из SA.CAT.001) — только если тема прошла через `topic-log.yaml` (WP-480) | Claude (R4) | — | `brief:` блок в записи темы `topic-log.yaml`, статус `accepted` |
