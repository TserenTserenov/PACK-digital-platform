...
**Критерий приёмки:** До публикации названы конкретный читатель, его обстоятельства, следующая работа и режим точности. Текст прошёл цикл автор → рецензент → исправление → повторная проверка; для визуального материала отдельно приняты текст, изображение и их композиция. Текст со status: ready публикуется в клуб в течение запланированного слота. Адаптации для соцсетей публикуются автоматически через S47. После публикации зафиксирован хотя бы один сигнал принимающего использования: читатель понял тезис, выполнил следующую работу или дал содержательное возражение. Автор получает уведомление о новом комментарии < 15 мин.
...
|---|-----|-----|--------|----------|
| 0 | Собрать контент-бриф: сегмент/тон, конкретный читатель, обстоятельства, следующая работа, режим точности (`wide`/`explanatory`/`engineering`) + мем P-NNN/M-NNN — только если тема прошла через `topic-log.yaml` (WP-480) | Claude (R4) | — | `brief:` блок в записи темы `topic-log.yaml`, статус `accepted` |
| 1 | Написать лонгрид для клуба (source-of-truth) | Автор (R14) + Claude (R4) | — | `{NNN}-1-club-{date}.md` (status: draft) |
| 2 | Проверить текст у рецензента, исправить и отдать на повторную проверку | Автор (R14) + Рецензент | — | status: draft → замечания → правки → повторный verdict |
| 3 | Создать адаптации для выбранных каналов | Claude (R4) | S46 | `{NNN}-{N}-{channel}-{date}.md` (status: draft) |
| 4 | Проверить адаптации; для визуальных форм отдельно принять текст, изображение и композицию | Автор (R14) + Рецензент | — | Правки → повторный verdict → готовые адаптации |
