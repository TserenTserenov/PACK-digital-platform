...
- Q1: где живёт `tools_cache` — in-memory или Redis/Neon? In-memory достаточно для бота с 1 instance; для multi-instance — нужен shared cache.
- Q2: какая роль владеет «discovery» — `R21 Publisher` или новая `R?? MCP Client Coordinator`? См. IntegrationGate Ф3.
- Q3: формат tier-фильтра — naming convention (`admin_*`) или явные теги (`tags: ["subscriber", "free"]`)? Решить с Aisystant MCP командой.
