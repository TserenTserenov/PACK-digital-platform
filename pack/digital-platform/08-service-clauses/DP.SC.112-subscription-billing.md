...
- **Переход T2→T3** бесплатный (заполнить ЦД), T3→T4 бесплатный (setup.sh или Gateway URL)
- **Платформенный API-ключ** — AI-вызовы (агенты, персонализация) включены в подписку БР и в триал. Лимит токенов по тарифу. Личный API-ключ не требуется. Direct MCP (T4): опционально свой ключ для неограниченного использования
...
| **Биллинг** | Включено в подписку БР. Trial: базовый лимит токенов. Подписка: расширенный лимит по тарифу |
| **Превышение лимита** | Уведомление → опция докупить пакет токенов или подключить свой API-ключ |
...
**Экономика:** себестоимость AI-вызовов ~$3-5/мес на активного пользователя (при оптимальном routing между моделями). При подписке $10-40/мес — маржа на AI-вызовах покрывается.
