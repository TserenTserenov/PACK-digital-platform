...
- **Подписка БР (T2–T4)** — одна подписка, включает все тиры и **полный доступ к Gateway** (ЦД, персональные знания, GitHub, unified search + всё из бесплатного уровня). Расширяет бот-сценарии: Лента, полноценные программы развития, уведомления, индивидуальные рекомендации. YooKassa (РФ) или Stripe (мир). Цена растёт постепенно, отдельных платных тиров нет
- **Переход T2→T3** включён в подписку (заполнить ЦД через Gateway), T3→T4 включён в подписку (Gateway URL или setup.sh)
- **Платформенный API-ключ** — базовый лимит AI-вызовов включён в подписку БР. T0/T1: базовый лимит для знакомства (бесплатно). Личный API-ключ не требуется
...
**Критерий приёмки:** Подписка работает через 3 провайдера (YooKassa, Stripe, TG Stars). Бухгалтер видит метрики. Менеджер может зарегистрировать ручной платёж.
...
| **Биллинг** | **Отдельный от подписки БР.** Базовый лимит включён в подписку. T0/T1: без лимита (только бесплатные инструменты). Превышение — отдельный биллинг |
...
| # | Шаг | Кто | Сервис |
|---|-----|-----|--------|
| 1 | Начинает с бесплатных бот-сценариев (T0/T1). Gateway knowledge_* доступен бесплатно | Участник | — |
| 2 | Выбирает тариф и оплачивает подписку (YooKassa/Stripe) | Участник | Billing Module |
| 3 | Расширяет доступ в соответствии с тарифом (T2): Лента, полноценные программы, полный Gateway | Платформа | Billing Module → Ory metadata |
| 4 | Автопродление или уведомление об окончании | Платформа | Billing Module (cron) |
| 5 | (Опционально) Отмена подписки | Участник | /cancel → grace 7 дней → T1 |
...
Подписка на всю жизнь — не метафора, а буквальная модель. Каждому нужно своё время для прохождения пути от Стажёра до Общественного деятеля. Платформа заменяет формальное образование: непрерывное развитие вместо разовой оплаты за диплом.
...
## Техническая граница T1 ↔ T2 (WP-210 Ф2a)
...
| Канал | T0/T1 (Ory auth) | T2 БР (оплачено) | T3/T4 БР |
|-------|-------------------|------------------|----------|
...
**Техническая реализация:** [DS-MCP/gateway-mcp/src/index.ts](../../../DS-MCP/gateway-mcp/src/index.ts) — `validateOryToken()` проверяет JWT, `hasSubscription` проверяется только для non-knowledge tools. Freemium-модель: knowledge_* доступны без подписки (T0/T1), dt_*/personal_*/search — только при подписке. Примечание: устаревший trial-флаг бота в этот процесс не попадает (trial отменён WP-349 Ф24).
...
- Связан с **DP.SC.116** (Уведомления) — payment confirmation, churn nudge
