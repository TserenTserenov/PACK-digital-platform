...
- Контролёр **запускается из единственного места** (systemd timer на tsekh-1, 06:00 МСК) — `pg_try_advisory_lock` гарантирует single-runner на shared БД.
...
**Триггер:** Ежедневный systemd timer 06:00 МСК на tsekh-1.

**Время отклика:** Обработка всех опт-инов завершается к 06:30 МСК (≤30 мин). Нудж в TG доставляется не позже 06:35 МСК.
...
| Сервис | Роль | Триггер |
|--------|------|---------|
| `iwe-stage-controller.py` (tsekh-1, systemd timer) | DP.ROLE.046 | ⏰ Ежедневно 06:00 МСК |
...
| # | Шаг | Кто | Действие |
|---|-----|-----|----------|
| 1 | Timer запускает контролёр | systemd | tsekh-1 06:00 МСК |
