...
status: draft
layer: L4-Personal
audience: [R1 Стратег, R5 Архитектор, R15 Применитель захватов, DP.ROLE.050 R33 Старатель паттернов, пилот (approve reflex-кандидата адаптера B)]
created: 2026-05-17
updated: 2026-06-21
links: [WP-295, WP-423, DP.SOTA.022, DP.SC.037, WP-272, DP.SC.024, executor-catalog.yaml]
related:
  extends: []
  realizes: []  # Ф4 WP-295
...
## Выходные адаптеры (WP-423: A — правило, B — рефлекс)

> Один движок добычи (DP.ROLE.050 R33 Старатель) — **два выходных адаптера**. Различаются ТОЛЬКО типом выхода, адресатом approve, классом приёмки и целевым файлом. Алгоритм добычи класса из повторов (кластер → ранк → pending-review → human-approve) и инвариант «никогда не auto-create» — общие. Различение в `distinctions.md`: «Движок добычи паттернов (R33) ≠ выходной адаптер кристаллизации».

| Ось | Адаптер A — правило | Адаптер B — рефлекс (кристаллизация) |
|-----|---------------------|--------------------------------------|
| Тип выхода | `AR.NNN.md` (поведенческое правило; читает агент в промпте) | строка в `executor-catalog.yaml reflexes:` (исполняемый код без LLM) |
| Адресат approve | R15 на apply-captures | пилот |
| Класс приёмки | extraction-report flow (текст читается) | двухуровневый smoke/replay (код исполняется) — см. ниже |
| Целевой файл | `PACK-agent-rules/rules/` | `DS-my-strategy/scripts/executor-catalog.yaml` |
| Upstream-режим | trace store (WP-295 Ф1) | trace store ИЛИ ручной (`gate-decisions.jsonl` + база косяков WP-316) |
| Статус | реализован (`scripts/trace-pattern-miner.py`) | ручной контур живой (`iwe_gate.py` + executor-catalog); авто-контур = Ф7 WP-423 |

**Класс приёмки адаптера B (двухуровневый — WP-423 peer-session 2026-06-21):**

- **candidate (`enabled: false`)** — минимальный bar: **smoke deny-fixture** на сигнатуру рефлекса. Это *over-match guard* — ловит false-positive (рефлекс срабатывает там, где не должен).
- **`enabled: true`** — обязательный bar: smoke **+ replay на истории**. Это *false-negative guard* — подтверждает, что рефлекс срабатывает там, где должен. Replay сейчас = **выборочный ручной** прогон последних N реальных входов из `gate-decisions.jsonl` / истории задач (не требует готовности WP-295 Ф2); при готовности WP-295 Ф2 (replay-инфраструктура) — апгрейд на автоматический полный replay. Апгрейд **усиливает guard, но не меняет обещание** — двухуровневость заложена с самого начала.
- **Тип-калибровка (какой guard критичен по типу рефлекса):**
  - **skip/no-op рефлекс** (пропустить дорогой шаг, напр. `skip-no-python-changes`): критичен false-positive (пропустил нужное) → over-match guard первичен; false-negative = лишний вызов модели = только стоимость, не корректность.
  - **action рефлекс** (детерминированное действие, напр. `apply_captures_sweep`): критичны оба направления → replay положительных кейсов **обязателен** для `enabled: true`. Если положительных кейсов в истории ещё нет — рефлекс остаётся `candidate` (не достижим `enabled: true`, и это корректно).
- **Grandfather:** рефлексы `source: bootstrap` предшествуют этому правилу. Enabled-bootstrap не блокируются — помечаются флагом «sampled-replay backfill» на ближайший Week Close. Candidate-bootstrap (`enabled: false`, напр. `apply_captures_sweep`) проходят полный класс приёмки при попытке `enabled: true` (fixture может отсутствовать у legacy-записи — добавляется на этом шаге).

**Negative-guarantee адаптера B:** reflex-адаптер **НЕ заменяет** runtime-гейт `iwe_gate.py`. Майнер R33 **ПРЕДЛАГАЕТ** строку `reflexes:` (write только в pending-review / candidate `enabled: false`); гейт **ИСПОЛНЯЕТ**. Майнер read-only по trace и по `gate-decisions.jsonl`. Держит «Контрольная роль ≠ Операционная роль»: R33 — контрольная (предлагает), `iwe_gate.py` — операционная (исполняет).
...
### SC.040.4 — Reflex-кандидат из gate-decisions / базы косяков (адаптер B)

**Триггер (per-item, continuous):** ≥3 повтора одного детерминированного **исполнимого** класса за 30 дней в `~/.iwe/gate-decisions.jsonl` ИЛИ суждение по базе косяков (WP-316, `iwe_memory.db`).

**Потребитель:** пилот (approve `enabled: true`).

**Владелец:** DP.ROLE.050 R33 Старатель паттернов (формирует reflex-кандидат; никогда не включает сам — инвариант never auto-create).

**Шаги:**
1. R33 / ручной разбор опознаёт детерминированный исполнимый класс в `gate-decisions.jsonl` или базе косяков.
2. Формируется строка `reflexes:` с `source: crystallized`, `enabled: false` (candidate) + smoke deny-fixture сигнатуры (over-match guard).
3. Пилот ревьюит candidate. Для `enabled: true` — replay на истории (см. класс приёмки адаптера B выше).
4. Пилот approve → `enabled: true`. Следующая похожая задача → `iwe_gate.py` → `routing=reflex` (без LLM).

**Время:** per-item, не batch (отличие от SC.040.1 weekly).

**Ожидаемый результат:** повторяющийся класс уходит из LLM-петли в детерминированный handler. Это контур самодостраивания WP-423 (ручной режим — Ф6.5; авто-режим — Ф7 при триггере ≥50 записей `reflexes:`).

**Симптом пропуска:** один класс крутится в LLM ≥3 раза/мес, но рефлекса нет → ручная кристаллизация на ближайшем Week Close.
