...
status: draft
layer: L4-Personal
audience: [R1 Стратег, R5 Архитектор, R14 Заказчик, R47 Детектор]
created: 2026-04-11
updated: 2026-04-11
...
6. **IntegrationGate обязателен для нового детектора.** Добавление детектора = добавление нового инструмента → 4 шага: обещание (описать в этом SC или новом) → 3 сценария использования → роль (R47 Детектор в DP.ROLE.001, если меняется контракт) → реализация. Пропуск = P10.
...
**Владелец:** R47 Детектор (детектор_incident.sh) + dispatcher + writer. Человек не участвует в фиксации.
...
**Владелец:** R5 Архитектор (дизайн) + R6 Кодировщик (реализация) + R47 Детектор (runtime после промоции).

**Шаги:**
1. **Gate:** Детектор должен пройти IntegrationGate — обещание (новый сценарий в этом SC или отдельный SC), 3 сценария, проверка что R47 описан в DP.ROLE.001. Пропуск → P10.
...
|--------|------|---------|------|
| S-TBD-capture-dispatcher | R47 Детектор | PostToolUse, Stop | `~/IWE/.claude/hooks/capture-bus.sh` |
| S-TBD-capture-writer | R47 Детектор | stdin event | `~/IWE/.claude/lib/capture_writer.sh` |
| S-TBD-detector-incident | R47 Детектор | PostToolUse | `~/IWE/.claude/detectors/detector_incident.sh` |
...
**Статус:** draft, 11 апр 2026. Реализация: Ф8.1-Ф8.3 WP-217 (dispatcher+writer+detector_incident v1). Retroactive документирование (паттерн P10 признан и пофикшен). Требует: (1) ревью владельца, (2) регистрации R47 Детектор в DP.ROLE.001, (3) промоции после обкатки 2 недели.
