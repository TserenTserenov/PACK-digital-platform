...
**Что получит:**
- При чекауте сервиса видит сумму доступной скидки: `min(копилка, потолок_по_квалификации) × курс`, где курс = `1 балл ≈ 0.875 ₽` (1¢ × 87.5 ₽/$, текущий курс ЦБ; владелец курса — DP.ECON.001 §6)
- Применяет скидку — баллы резервируются в `redeemed_events(status='reserved')`, ЮКасса видит уменьшенную сумму
- После успешной оплаты (`payment.succeeded` webhook) — резерв подтверждается (`status='confirmed'`), копилка уменьшается
- При отмене (`payment.canceled` или timeout 30 мин) — резерв откатывается (`status='rolled_back'`), копилка восстанавливается
- В истории видит каждое списание с привязкой к payment_id
...
- Активность: 10ч/нед обучения за последние 4 недели → накоплено ~1400 баллов
- Покупка: семинар 5000 ₽
- Ожидаемая скидка: `min(1400, 100) × 0.875 = 87.5 ₽` (1.75% от цены семинара)

**Эталонный сценарий** для пилота высокой степени:
- Пилот: Реформатор (ст. 7, ×4.0, daily_cap 700)
- Активность: 20ч/нед × 4 нед = ~22 400 баллов накоплено
- Покупка: семинар 5000 ₽
- Ожидаемая скидка: `min(22400, 700) × 0.875 = 612.5 ₽` (12% от цены)
