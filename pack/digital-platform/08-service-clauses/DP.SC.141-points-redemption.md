...
**Что получит:**
- При чекауте сервиса видит сумму доступных **бонусов**: `min(баланс_баллов, Σ(active_days_at_qual_i × cap_i)) × курс`, где курс = `1 бонус = 0,10 ₽` (владелец курса — DP.ECON.001 §6).
...
### Сценарий 1: Оплата семинара рублями со скидкой бонусами

| # | Шаг | Кто | Сервис |
|---|-----|-----|--------|
| 1 | Открывает витрину семинаров (`/showcase`), выбирает семинар (цена 5000 ₽) | Участник | Бот |
| 2 | Видит: «Доступно: 12 ₽ скидки из 1400 бонусов (потолок: Ученик ×1.0 = 100 баллов/день)» | Участник | Burn-эмиттер: SELECT доступная скидка |
...
### Сценарий 2: Полная оплата бонусами (100% скидка)

| # | Шаг | Кто | Сервис |
|---|-----|-----|--------|
| 1 | Участник со ст. Реформатор ×4.0 (потолок 700 баллов/день, копилка 6857 бонусов) выбирает занятие за 100 ₽ | Участник | Бот |
| 2 | Видит рублёвую скидку по курсу `0,10 ₽/бонус` (например, 1000 бонусов = 100 ₽) | Участник | Burn-эмиттер |
...
DP.ECON.001 §6 «Конвертация: баллы → скидки» декларирует текущий курс `1 бонус = 0,10 ₽`. Рабочий источник истины — активная запись `reference.loyalty_pool_config.rate`; резервное значение бота обязано совпадать с ней.
