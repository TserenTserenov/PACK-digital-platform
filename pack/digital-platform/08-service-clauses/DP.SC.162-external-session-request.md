...
# [DP.SC.162] Внешняя рабочая сессия
...
Расширение scope через новые MCP-инструменты не требует изменения SC.162 — только обновления DP.ROLE.061 §Capability.
...
|-----------|------|-----------|
| Ingress Adapter (aist-bot, cloud) | DP.ROLE.061 §Ingress | Принять сообщение, авторизовать, дописать в SESSION-thread |
| Egress Adapter (dispatcher, local) | DP.ROLE.061 §Egress | Обнаружить новый ход, запустить Claude Code с полным thread, вернуть ответ |
