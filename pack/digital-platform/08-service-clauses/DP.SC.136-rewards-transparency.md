...
- На запрос `/points` в боте — структурированный ответ из трёх блоков:
  1. **Два числа баланса:**
     - **Баллы заработано (всего):** `earned_total` — никогда не убывает, лидерборд. *(До добавления `earned_total`-колонки: `point_balances.points` + сумма потраченных бонусов из `redeemed_events` — приблизительно)*
     - **Бонусы (доступно к оплате):** `min(points, Σ(active_days_at_qual_i × daily_cap_i))` — можно потратить на скидку
