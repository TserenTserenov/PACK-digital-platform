...
**Зачем:** IWE — сложный инструмент. Без регулярного обслуживания он деградирует: memory раздувается, различения устаревают, правила мертвеют, Pack расходится с DS, docs расходится с кодом. «Заточка пилы» (Кови) — аналог элемента #14 культуры работы IWE (Эволюция системы) — регулярное обновление инструмента, без которого работа деградирует. При этом у владельца жёсткий cognitive budget, и наивный календарный чеклист из 22 пунктов потопит любой бюджет за 2 недели.
...
5. **Агент: ТО памяти (T) — проверка статической нагрузки контекста:**
   - `distinctions.md` строк > 80 → drift-флаг, предложить аудит в WP-7 Ф-MEM (нарушение DP.KR.001 §6)
   - `MEMORY.md` строк > 200 → флаг, предложить архивацию старых feedback
   - `memory/*.md` без обращения > 14 дней > 5 файлов → кандидаты на понижение horizon warm
   - Флаги информативны — человек принимает решение
6. Человек: R-вопросник (5-7 вопросов) → ответы в Week Report.
7. Человек: принимает решения по drift, staging, флагам ТО памяти.
**Время:** 20-30 мин.
**Симптом пропуска:** staging заморожен ≥2 недель с `validated`, drift растёт, Week Report без R-ответов, distinctions.md > 80 строк без флага в отчёте уже 2+ недели.
