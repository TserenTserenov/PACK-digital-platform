...
> **Примечание (WP-478, 11.07.2026):** имя «Непрерывное обучение» оставлено как домен-термин (архитектурный идентификатор Service Clause), не user-facing текст. Переформулировка пользовательских сообщений (WP-478) его не переименовывает — переименование самого SC было бы отдельным архитектурным решением с широким blast radius по кросс-ссылкам Pack.

> **Уточнение (инверсия модели, WP-415, 2026-06-14).** Метод обучения принадлежит платформе и не зависит от конкретного клиента-помощника. IWE — один из каналов реализации; руководства и обратная связь доступны через любой совместимый клиент.
