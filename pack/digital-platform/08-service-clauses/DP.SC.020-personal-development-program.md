...
> **Уточнение слоя (инверсия модели, WP-415, 2026-06-14).** Сервис программы развития (роли Диагност, Оркестратор, Портной, Навигатор + цифровой двойник) — платформенный, не привязан к конкретному клиенту. IWE — один из каналов входа; пользователь может получать программу развития через любой совместимый клиент-помощник.
...
| 4 | **Доставка** | Синхронизатор | Занятие отправляется по расписанию (утро, feed_schedule_time) | Бот: сообщение с занятием, вопросом, заданием |
| 5 | **Изучение занятия** | Пользователь | Читает материал, осмысляет | Бот: текст + bridge + retrieval |
...
## Канал персонального руководства (актуальная реализация, MVP-2026-W18+)

> **Контекст:** При продумывании канала «Экзокортекс» (выше) предполагалось, что у каждого пользователя будет полноценный репо `DS-learning-workbook` с профилем + заданиями + дневником + артефактами. Реальная имплементация (WP-245 Block B, deploy 4 мая 2026) — упрощённый плоский репо `personal-guide` с генерируемыми файлами `daily/`, `weekly/`, `monthly/`. Этот раздел описывает **то, что работает сейчас** в production у пилотов; раздел «Канал Экзокортекс» выше остаётся целевым видением.

### Обещание тёплого канала

**Кому:** Пилот программы ЛР с активной БР (Базовая Резидентура) подпиской и личным GitHub-аккаунтом.

**Что получит:**
- **Каждое утро** в его репозитории `<github-login>/personal-guide` появляется свежий файл с заданием на день/неделю/месяц
- **Содержание адаптировано** под его текущую ступень (FORM.080), RCS-профиль (FORM.089, 7 слотов), bottleneck и monthly-theme
- **Decision-log inline в файле** — Портной объясняет: какую область выбрал и почему, какой элемент и почему
- **B-lite fallback** — если RCS пуст (новый пилот), Портной использует FORM.093 graduate_profile (stage + path) до накопления данных

**Когда:**
- **Weekly:** ПН 05:00 EEST → `weekly/{YYYY-Www}.md`
- **Daily:** Вт-Вс 06:00 EEST → `daily/{YYYY-MM-DD}.md`
- **Monthly:** ПН первой недели месяца → `monthly/{YYYY-MM}.md` (опц., при наличии monthly-theme от пользователя в `config/monthly-theme.md`)

**Свидетельства:**
1. Файл существует в репо (commit hash в `git log`)
2. Frontmatter содержит `stage`, `bottleneck`, `generated_at`
3. Body содержит lesson_plan + content + decision_log

### Архитектура (текущая)

```
            ┌──────────────────────────────────────┐
            │   Neon indicators schema (Память)    │
            │  • calculated_profile (RCS, FORM.093)│
            │  • digital_twins (rcs_history)       │
            │  • learning.domain_event (7 дней)    │
            └─────────────────┬────────────────────┘
                              │ psycopg2 read (RLS read-only)
                              ▼
            ┌──────────────────────────────────────┐
            │   tsekh-1 (NixOS systemd timer)      │
            │   render-pilot-guides.py             │
            │   + agents/tailor/planner.py         │
            │   + agents/tailor/horizons.py        │
            │   + agents/tailor/prompt.md (Sonnet) │
            └─────────────────┬────────────────────┘
                              │ GitHub App JWT
                              ▼
            ┌──────────────────────────────────────┐
            │  github.com/<login>/personal-guide   │
            │  ├── daily/YYYY-MM-DD.md             │
            │  ├── weekly/YYYY-Www.md              │
            │  ├── monthly/YYYY-MM.md              │
            │  └── config/monthly-theme.md (opt.)  │
            └─────────────────┬────────────────────┘
                              │ git pull
                              ▼
            ┌──────────────────────────────────────┐
            │  Пилот: VS Code + Claude Code (PILOT)│
            │  Бот /personal-guide → ссылка на     │
            │      свежий файл                     │
            └──────────────────────────────────────┘
```

**Носитель роли R27 «Портной»:** Python-скрипт (см. `DS-autonomous-agents/agents/tailor/CLAUDE.md` § «Носители роли R27»). Будущий носитель — ИИ-агент с tool-use циклом через MCP Gateway, blocked by WP-150 Ф6.

### Различение `personal-guide` vs `DS-learning-workbook`

| | `personal-guide` (MVP, реализовано) | `DS-learning-workbook` (целевое видение) |
|---|---|---|
| **Структура** | Плоский: `daily/`, `weekly/`, `monthly/`, `config/` | Иерархический: `current/`, `progress/`, `profile/`, `archive/` |
| **Содержание** | Только сгенерированный контент | Контент + дневник + артефакты + ответы + диагностика |
| **Двусторонняя запись** | Только Портной пишет (через GitHub App) | Портной пишет в `current/`, пользователь — в `progress/` |
| **Имя** | Константа `personal-guide` (HD: «Имя репо ЛР = константа, не уникальный per-user») | `DS-learning-workbook` (с возможным per-user суффиксом) |
| **Статус** | ✅ active с 2026-05-04 | ⏳ будущая эволюция |

> **Принцип миграции:** `personal-guide` — узкий MVP для проверки гипотезы тёплого канала. По мере роста ИТ-уровня пилота (FORM.080 ст. 3+) и появления зрелого ИИ-агента-носителя — структура расширяется в сторону `DS-learning-workbook` без переименования репо. Repo остаётся одним.

### Триггеры рендера

| Триггер | Когда | Что генерируется |
|---------|-------|------------------|
| **scheduled.weekly** | ПН 05:00 EEST (cron) | `weekly/{YYYY-Www}.md` для всех активных пилотов |
| **scheduled.daily** | Вт-Вс 06:00 EEST (cron) | `daily/{YYYY-MM-DD}.md` для всех активных пилотов |
| **manual** | `render-pilot-guides.py --user <login>` | Force-rerender для конкретного пилота |
| **future: orchestrator-driven** | WP-203 Ф1.5 | По 7 типам триггеров коррекции (slot_miss, hypothesis_fail, ...) |

### SLA и failure modes

- **Время отклика:** руководство появляется в репо до 06:30 EEST (запас 30 мин на ретраи)
- **Idempotency:** повторный рендер за тот же день перезаписывает с архивом в `history/`
- **Failure mode «B-lite stuck»:** если RCS пустой более 14 дней — Портной продолжает генерировать на B-lite fallback (FORM.093 stage+path); запись в `decision_log: source = "form093_fallback"`
- **Failure mode «GitHub App auth fail»:** scheduler пишет в alert; пилот не получит файл, но соседние пилоты не блокируются

### Связь с WP-287 (тёплый канал документации)

[WP-287](../../../../../DS-my-strategy/inbox/WP-287-iwe-usage-guide.md) Ф2 «Тёплый канал — контракт с Портным» зависит от этого SC. Контракт WP-287 ↔ Портной = **этот раздел**:
- Какие шаблоны для каждой ступени → ссылка на WP-245 (нарративная архитектура)
- Как ЦД управляет выбором → §«Архитектура (текущая)» выше: RCS + bottleneck + monthly-theme
- Refresh schedule → §«Триггеры рендера»
- Q2 fallback → B-lite на FORM.093 (см. failure modes)

WP-287 Ф2 не требует отдельного SC — достаточно ссылки на этот раздел SC.020.
...
- **Реализуется через WP-222** (Портной как роль + текущий носитель) и **WP-245 Block B** (render-pilot-guides.py + scheduler)
- Контракт чтения Памяти.Derived: **PD.SPEC.001** (формат tailor_context)

---

## Цикл ВДВ (Вход-Действие-Выход) в персональном руководстве

> **Контекст:** Персональное руководство работает по циклу ВДВ v9 (METHOD.008 — 7-стадийный каскад), адаптированному под серверную генерацию. Стадии 1–4 выполняются автоматически (Портной-2, tsekh-1), стадия 5 — пилот вручную. Полное описание процесса: [DS-autonomous-agents/PROCESSES.md §S60](../../../../../../DS-autonomous-agents/PROCESSES.md).

### 5 шагов цикла

| # | Стадия ВДВ | Что происходит | Когда | Результат |
|---|-----------|---------------|-------|-----------|
| 1 | **Вход** | Сбор RCS (7 слотов), событий за 7 дней, рефлексий пилота (`history/*-reflection.md`), monthly-theme | Перед каждым рендером | Полный контекст для Портного-2 |
| 2 | **Действие: Планирование** | Оркестратор решает режим (weekly/daily). Портной-2 строит горизонты (quarter→month→week→day) | Серверная сторона | `trigger_info` + `horizon_ctx` |
| 3 | **Действие: Генерация** | Claude Sonnet генерирует Markdown-файлы: README, profile, worldview, methods, weekly/, daily/ | Серверная сторона | 6 файлов в RAM |
| 4 | **Выход: Публикация** | GitHub API: commit + push в `personal-guide`. Архивация прошлых weekly/daily в `history/` | Серверная сторона | Репо обновлено |
| 5 | **Потребление** | Пилот читает файлы, выполняет занятие, пишет рефлексию | Ежедневно | Рефлексия → feedback loop |

### Два режима рендера

**Weekly (Пн 05:00 EEST) — полная пересборка:**
- Генерируются все 6 файлов
- Архивируются прошлые `weekly/` и `daily/` в `history/`
- Используются все 4 горизонта (quarter/month/week/day)
- Подходит для планирования недели

**Daily (Вт–Вс 06:00 EEST) — компактное обновление:**
- Генерируется только `daily/<дата>.md`
- Компактный контекст (только day-горизонт)
- Быстрее, дешевле (меньше токенов)

### Feedback loop (замкнутый контур)

Пилот пишет `history/<YYYY-MM-DD>-reflection.md` (5 вопросов). Портной-2 читает эти файлы при следующем рендере:

- **Q3 «Что узнал»** → сигнал для PD.FORM.087 (фазовый переход). Если пилот фиксирует освоение ключевого различения — это может триггерить смену ступени.
- **Q5 «Что завтра»** → намерение пилота. Передаётся в `horizon_ctx.pilot_reflection_tomorrow_intention` и влияет на акцент следующего `daily/`.

Это создаёт **замкнутый контур**: пилот пишет → Портной читает → следующее руководство адаптируется → пилот читает.

### Отличие от ботового цикла (таблица выше)

| | Ботовый цикл (9 шагов выше) | ВДВ-цикл personal-guide |
|---|---|---|
| **Канал** | Telegram (push) | GitHub repo (push) + VS Code (pull) |
| **Формат** | Сообщение в чат | Markdown-файлы |
| **Рефлексия** | Ответ в бот (оценивает Проводник) | Файл `history/*-reflection.md` (читает Портной-2) |
| **Архивация** | Neon (learning_history) | Git (history/) + Neon (events) |
| **Автономность пилота** | Низкая (бот ведёт) | Высокая (пилот сам открывает файлы) |
| **ИТ-уровень** | 0–2 | 2–3 |
| **Ступени** | 0–3 (основной) | 2–4 (основной) |

> **Переход:** Пилот на ступени 2+ с ИТ-уровнем ≥2 может перейти с ботового канала на personal-guide. Бот остаётся как fallback (уведомления + reactive Навигатор).

---

*Создан: 2026-03-22. Обновлён: 2026-03-23 (добавлена архитектура канала «Экзокортекс»). Обновлён: 2026-05-06 (WP-222 Ф5: добавлен раздел «Канал персонального руководства»). Обновлён: 2026-05-14 (WP-149 Ф6: добавлен раздел «Цикл ВДВ» — 5-шаговый каскад Портного-2 с feedback loop).*
