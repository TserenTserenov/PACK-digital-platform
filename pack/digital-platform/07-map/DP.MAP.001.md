...
> Auto-generated from frontmatter on 2026-09-25. Do not edit manually.
...
| DP.M.454 | Онбординг нового кандидатского критерия в действующую рубрику: обязательное заполнение + исключение из вердикта + обязательный комментарий при худшей оценке | — | active |
| DP.M.455 | Гранулярность lock'а по инструменту, а не по сервису целиком | — | draft |
| DP.M.456 | Первичный артефакт и сопроводительные производные — синхронизировать в одном коммите | — | draft |
| DP.M.457 | Таблица «Результат | Факт | Зависит от» вместо вертикальной очереди результатов | — | active |
| DP.M.458 | Понижение тарифа как отложенный переход на границу расчётного периода (pending_tariff + switch_at) | — | active |
| DP.M.459 | Причина выдачи права как поле первого класса (grant_source): ограничения объявляются на тип происхождения, не на подписку вообще | — | active |
| DP.M.460 | Автоматическая миграция/повышение тарифа через allowlist допустимых исходных состояний, не через исключение нового кода | — | active |
| DP.M.461 | Портируемый in-place патч файла под уже удерживаемой блокировкой: temp-файл в той же директории + mv вместо `sed -i` | — | active |
| DP.M.462 | Три уровня «готово» для свидетельства: собран живой факт → закрыта строка реестра → вынесен вердикт по ставке | — | active |
| DP.M.463 | Проверка исполнимости критерия фальсификации до его фиксации: существует ли путь, по которому запланированная боевая проверка вообще дойдёт до проверяемого механизма | — | active |
| DP.M.464 | SECURITY DEFINER-функция для вызывающего без личности конкретного читателя — не ослабление RLS-политики | — | active |
| DP.M.465 | Байт-точное сравнение дерева как доказательство публикации хрупко — проверять сохранность своей правки: структурная привязка вставки или трёхстороннее слияние по типу файла | — | active |
| DP.M.466 | Карта навигации по формированию персонального руководства | — | active |
| DP.M.467 | Read-only токен не защищает от подмены исполняемого клиента — нужна защищённая цепочка запуска | — | active |
| DP.M.468 | Строгая проверка целостности всего набора выполняется до цикла по частным доказательствам | — | active |
| DP.M.469 | Условное значение генерируемого артефакта разрешать один раз в генераторе, а не веткой внутри heredoc | — | active |
| DP.M.470 | Самовосстановление прерывает зависший rebase только при доказанном владельце состояния | — | active |
| DP.M.471 | Терминальный статус подтверждают два ортогональных канала доказательства | — | active |
| DP.M.472 | Батчинг уведомлений через durable pending-очередь, очищаемую только после подтверждённой доставки | — | active |
...
| DP.FM.437 | Форма контракта стороннего API предположена, а не проверена: имя поля не то, значения нормализуются сервером при записи — вскрылось на первом боевом прогоне, поймано собственным ассертом схемы до порчи данных | — | draft |
| DP.FM.438 | Непроверенный код возврата канонизатора внутри сравнения: сбой превращается в пустую строку с ОБЕИХ сторон — сравнение молча признаёт совпадение там, где не состоялось вовсе | — | draft |
| DP.FM.439 | Проверка формы закрыла транспортный конверт (JSON-RPC envelope), но не доменный payload внутри — уровни контракта не вложены друг в друга, и дрейф поля у одного участника обнулил batch-отчёт остальных | — | draft |
| DP.FM.440 | Fail-safe гейт закрытия верно отказывает легитимному, но нестандартному пути, для которого не спроектирован путь восстановления | Корректный отказ гейта — не доказательство его полноты: каждый новый легитимный путь к готовности (новый путь вызова, ручное восстановление после конфликта) должен быть добавлен в распознаваемые случаи или получить спроектированный путь восстановления; иначе гейт копит блокировки легитимной работы, а композиция двух корректных guard'ов оставляет ноль разрешённых ходов. | draft |
| DP.FM.441 | Обёртка публикации довезла до origin только последний коммит цепочки и вернула полный успех — второй независимый триггер класса «успех при частичном результате» после закрытия первого | Закрытие одного найденного триггера у бага класса «операция сообщает успех при частичном результате» не закрывает класс: пока точка успеха не проверяется независимо от внутреннего пути (сверка итогового состояния origin с ожидаемым по patch-id или содержимому), та же обёртка может соврать другим, ещё не найденным способом. | draft |
| DP.FM.442 | Session-level search_path в закешированном пуле соединений маскирует существующие таблицы под «relation does not exist» | — | draft |
| DP.FM.443 | Сужение scope idempotency-ключа для починки гонки блокировок — компенсация не того слоя (near-miss) | — | draft |
| DP.FM.444 | Частотный троттлинг алертов без порога по длительности инцидента скрывает многосуточный простой | — | draft |
| DP.FM.445 | Inline style на обёрточном div в marp-слайде: DOM верный, визуально не применяется | — | draft |
| DP.FM.446 | Смешанная публикация сессии (общий чекаут + worktree) даёт два SHA одного коммита | Часть коммитов сессии уходит в origin напрямую из общего чекаута, часть — через изолированный worktree-шлюз (DP.M.414); первый коммит пересоздаётся в origin под новым SHA, локальная ветка общего чекаута расходится с origin, инструменты подсчёта по локальному git log недосчитывают сессию. | draft |
| DP.FM.447 | Верный диагноз проверки не делает верной рекомендацию — совет дан без сверки с уже решённым тем же блокером в соседнем РП | Три агента подтвердили диагноз («защита сработала штатно») и на этом же основании дали рекомендацию («завести новое приложение»), не проверив карту открытых решений соседнего РП, где тот же блокер уже разобран иначе. | active |
| DP.FM.448 | Проверка чистоты рабочей копии перед закрытием считает собственные gitignore-артефакты конвейера «грязью» | Гейт «дерево чистое, включая ignored/untracked» отказывает на артефактах, которые сам защищаемый конвейер штатно производит (__pycache__, lock-файлы) — переход в защитный тупик становится маршрутом по умолчанию, а не редким исключением. | active |
| DP.FM.449 | Правило восстановления «вызывающий — живой потомок PID владельца» структурно невыполнимо при retry из планировщика | Условие reclaim требует, чтобы текущий вызывающий процесс происходил от PID предыдущей попытки — при retry из планировщика это невозможно по определению (старый процесс уже завершён), поэтому единственный автоматический путь восстановления всегда fail-closed. | active |
| DP.FM.450 | Гейт «отказать на грязной канонической копии» стоит на пути, где каноническая копия структурно никогда не чистая | Защитный гейт, верно резолвящий канонический путь, отказывает на условии «копия чистая», которое на общем живом чекауте не выполняется никогда, а не изредка — шесть плановых запусков отказали подряд, а подсказка в сообщении об ошибке называла несуществующий флаг. | active |
| DP.FM.451 | Health-check с бинарным исходом молча схлопывает «не смог проверить» в «всё в порядке» при недоступности сети | Функция проверки отставания от origin возвращала «ok» и при реальном «нет расхождения», и при невозможности вообще выполнить fetch (нет сети) — двухзначный статус защитной проверки не различает эти случаи, поэтому сбой самой проверки маскируется под её положительный вердикт. | active |
| DP.FM.452 | Валидатор сохраняет обязательность поля, которое производитель намеренно перестал писать после переноса шага — отвергает 100% свежих артефактов конвейера | Недельный бюджет часов перенесли с генерации плана на более ранний шаг стратегической сессии; генератор плана перестал его писать. Валидатор плана не узнал о переносе и остался с контрактом «поле обязательно», поэтому с этого дня отвергал каждый свежий план — ловил не дефект плана, а собственную рассинхронизацию с процессом. | active |
| DP.FM.453 | «Перенести код как есть в новый сервис» неявно предрешает открытую архитектурную развилку и умалчивает, куда логика пишет состояние | Предложение разработчика по логике тарифов было полно по бизнес-правилам и пусто по хранению: формулировка «перенести/сделать как есть» неявно закрывает открытую развилку (где живут данные и оба тарифа после переноса), не называя этого решением, и одновременно ничего не говорит о том, куда логика пишет состояние — отсутствие упоминания хранилища в предложении, меняющем состояние подписки, само по себе сигнал, а не «техническая деталь на потом». | active |
| DP.FM.454 | Quote-grounding проверяет наличие цитаты у замечания, не то, что цитата подтверждает именно это замечание | Фильтр обоснованности замечаний ДЗ-чекера проверял непустоту поля quote, не то, что содержание замечания реально следует из этой цитаты — живой post-deploy smoke-тест поймал настоящую, но не по теме, цитату, прошедшую мимо фильтра рядом с прежним ложным замечанием. | active |
| DP.FM.455 | Whitelist для sourcing-команды матчит текст команды, не резолвленное на момент исполнения значение переменной | Whitelist для `source|. "$HOME/.iwe-paths"` в dry-run-gate.sh допускал сорсинг только этого конкретного файла — но проверял синтаксическую форму команды, не итоговое резолвленное значение $HOME на момент исполнения; присваивание HOME= где угодно раньше в той же составной команде отзывает намерение whitelist без изменения его текстового паттерна. | active |
| DP.FM.456 | Whitelist по статическому текстовому маркеру спуфится буквальным вводом атакующего | Первая версия whitelist для .iwe-paths помечала легитимную форму статической строкой-маркером в тексте — атакующий, зная маркер (он статический, не секрет), может вставить его буквально в свой ввод и получить allow без реального совпадения намерения; фикс заменил маркер на nonce, генерируемый заново на каждый процесс. | active |
| DP.FM.457 | Под pipefail `echo большого_значения | grep -q шаблон` может дать ложный отказ на SIGPIPE | week-open-проверки использовали `echo "$PLAN_BLOCK" | grep -q pattern` под `set -o pipefail`; grep -q выходит по первому совпадению, echo, ещё дописывающий вывод, получает SIGPIPE — и весь конвейер иногда читается как false при реальном совпадении. Флакало на большом входе (800-строчный план W37). | active |
| DP.FM.458 | Построчный парсер YAML-подобного манифеста без стоп-условия на границе секции продолжает матчить паттерн вложенной секции внутрь соседней | parse_manifest() не имел стоп-условия на заголовке activity_checks: и продолжал матчить /^  - id:/ внутри этой секции тоже — порождая бракованные записи с пустыми source/derived, резолвящиеся в корень workspace и запускающие двойное дорогое сканирование. Подтверждено живой сверкой: 6 записей (2 бракованных) до фикса, 4 корректных после. | active |
| DP.FM.459 | Обёртка-классификатор пропускает `nice` по буквальному первому токену — собственный необязательный аргумент приоритета и вызов через абсолютный путь обходят блокировку | Классификатор команд гейта пропускал известные обёртки (env/nohup/time/sudo/nice) по буквальному имени первого слова. Первый проход фикса добавил nice, но не учёл, что (а) nice принимает необязательный аргумент приоритета (-n 19), из-за чего цикл останавливался на этом аргументе, не доходя до реальной команды, и (б) сравнение по буквальному токену не ловит вызов через абсолютный путь (/usr/bin/nice). | active |
| DP.FM.460 | `trap ... RETURN` внутри bash-функции не привязана к границе этой функции — переживает её и срабатывает повторно на следующем return | Функция ставила `trap ... RETURN`, ссылаясь на свои локальные переменные, в расчёте на срабатывание один раз при выходе именно из неё. В bash trap RETURN не имеет такой границы — он остаётся активным глобально и срабатывает на возврате из ЛЮБОЙ последующей функции, включая выполняемые намного позже в другом вызове хука. | active |
| DP.FM.461 | Молчаливый fallback на «путь не найден» маскирует всегда-ломаный primary path | day-plan.md шаг 0 искал day-open-scaffold.sh по устаревшему пути ($IWE_WORKSPACE/scripts/ вместо $IWE_SCRIPTS) — генератор скелета DayPlan не находился НИ РАЗУ, всегда срабатывал заранее подготовленный fallback (минимальный DayPlan с PENDING-маркерами), достаточно правдоподобный, чтобы никто не заметил постоянную поломку primary path. | active |
| DP.FM.462 | Retry на любую сетевую ошибку клиента совмещает «запрос точно не дошёл» с «неизвестно, дошёл ли» — повтор во втором случае дублирует побочный эффект | Общая функция отправки уведомлений повторяла запрос до 3 раз на любую сетевую ошибку curl, не различая exit 6/7 (DNS/connect — запрос до сервера не дошёл, повтор безопасен) и exit 28 (таймаут ответа — запрос мог быть уже принят и обработан, повтор даёт at-least-once дубль). Живой инцидент: три идентичных сообщения «Осиротевшая сессия» подряд под нагрузкой 7+ параллельных сессий. | active |
| DP.FM.463 | Безусловный `|| true` внутри общей функции глотает код возврата для всех вызывающих — место вызова теряет собственную ветку обработки сбоя | Новая общая обёртка дедупа уведомлений (`tg_notify_dedup`) содержала собственный `|| true` — код возврата глушился для всех вызывающих, включая тот, у которого ниже стояла своя ветка `|| log "primary alert delivery failed"`. Общая функция тихо забрала у места вызова решение «критичен ли этот сбой». Найдено холодным ревью до деплоя. | active |
| DP.FM.464 | `set -e` на захвате вывода обрывает блок до его собственной ветки «пропустить, не блокировать» — решение забирает оболочка, а не семантика вызываемого скрипта | Вызываемый скрипт осознанно возвращает ненулевой код ВМЕСТЕ с валидным выводом, передавая вызывающему решение «блокирует это или нет». Вызывающий захватывает вывод через `VAR=$(cmd)` под `set -e` — сам факт ненулевого кода обрывает блок ДО того, как исполнится его собственная пропускающая ветка. Прецедент: `ke-queue-stats.sh` возвращал 1 при непустых diagnostics (30 legacy-отчётов) с валидным JSON; `day-open` падал «1/23 blocks failed» четыре дня подряд (13–16.09). | active |
| DP.FM.465 | Тестовый гейт воспроизводит код, но не структуру путей целевого деплоя — детерминированный отказ там, где прод исправен | Прод собирает финальную раскладку каталогов во время деплоя (Nix rsync: хранимый в git `claude-lib/` → исполняемый `.claude/lib/`), поэтому расхождение имён в проде невидимо. Тестовый гейт гонял ~15 скриптов прямо из чекаута, где каталога с точкой ещё нет, — отказ был не флаки, а гарантированным на любом изменении любого из зависящих скриптов. | active |
| DP.FM.466 | Гейт доставки гоняет тесты по рабочему дереву общего чекаута, неявно предполагая тождество с HEAD — чужое застрявшее состояние блокирует всех | `sync-extensions-auto.sh` исполнял тесты прямо в общем чекауте `~/IWE`. Два тестовых файла с 14.09 лежали незакоммиченными (откат к до-коммитному состоянию, не чья-то новая работа, ни одним семафором не заявлены) — тот же тест давал 61 OK на чистом `origin/main` и 3 FAIL на грязном дереве. Гейт падал 19 попыток подряд (38 отмен в истории), блокируя доставку ВСЕХ изменений с 14.09 22:07, включая никак не связанные с этими файлами. | active |
| DP.FM.467 | Более ранний гейт (фильтр источника, форматная проверка) экранирует проверяемый внутренний механизм: негативный тест отказывает по чужой причине либо невыполним без ослабления защиты | Защита выстроена слоями, и более ранний гейт отбрасывает запрос раньше, чем тот дойдёт до проверяемого внутреннего механизма, — негативный тест получает отказ, но по чужой причине. Пример 1: IP-allowlist ЮKassa стоит в цепочке раньше кода квоты — запрос с адреса вне списка отбрасывается первым слоем, живой негативный тест квоты извне невозможен без ослабления внешнего фильтра. Пример 2: тест безопасности «нет отката на слабый путь» подавал не-UUID строку — её отбрасывала форматная проверка раньше проверяемой защиты, и регрессию отката тест бы не заметил. | active |
| DP.FM.468 | RLS-политика ссылается на GUC-функцию, у которой нет живого писателя переменной, — синтаксически верна, структурно не может сработать | Новая схема хранилища переписки наставник-участник переиспользовала существующую RLS-функцию app.require_account_id() вместо новой политики, но ни один код-путь не устанавливает нужный GUC для этого пути — политика подключена и синтаксически корректна, но структурно не может отфильтровать ни одной строки. Авторы честно пометили состояние DORMANT прямо в миграции, а не выдали его за рабочую защиту. | active |
| DP.FM.469 | Докстринг обещает «тихий пропуск» строки, реализация перехватывает только один тип исключения | Backfill-скрипт ловит только ForeignKeyViolationError; любое другое исключение asyncpg на конкретной строке роняет весь скрипт и прерывает перенос остальных строк — расходится с docstring, обещающим тихий пропуск плохих строк. На 100 чистых пилотных строках не проявилось; разрыв нашла независимая проверка кода холодным субагентом, не собственный успешный прогон. Побочный риск: непойманное исключение может напечатать значение поля в traceback (потенциальная утечка PII в лог). | active |
| DP.FM.470 | Пост-правочная проверка «old_string отсутствует на диске» ложно срабатывает на правке-продолжении | Хук пост-правочной проверки требовал полного исчезновения old_string с диска после правки — верно для замены, но ложно для правки-продолжения (new_string содержит old_string как подстроку, например «Ф44» → «Ф44/Ф45»): корректная правка получала ложный write_verify_failed. «Старый текст отсутствует» и «текст испорчен или потерян» совпадают только на правках-заменах. | active |
| DP.FM.471 | Парсер heredoc внутри command substitution не понимает вложенный контекст кавычек — необработанное исключение вместо деградации блокирует все последующие вызовы инструмента | Разбор heredoc внутри command substitution с вложенными кавычками не покрывал редкий, но валидный синтаксис: вместо мягкой деградации парсер security-хука поднимал необработанное исключение (SystemExit), и блокировался каждый следующий вызов Bash-инструмента, пока форма команды не изменится. Попытки разобрать точнее исторически вносили регрессии; безопаснее деградировать к текстовому сканированию. | active |
| DP.FM.472 | Fallback на альтернативную реализацию проверяет только код возврата — пропускает случай «процесс успешно завершился, но вернул пустой/испорченный результат» | Bash-fallback на python3-реализацию срабатывал только при ненулевом коде возврата. Платформо-зависимое декодирование портило кириллический заголовок в пустой slug, но python3 завершался с кодом 0 — fallback не срабатывал, пустой slug уходил дальше. Успешное завершение процесса — необходимое, но не достаточное условие пригодности результата. | active |
| DP.FM.473 | Разбор таблицы по фиксированному индексу колонки молча ломается при любом изменении состава колонок | day-open-priorities-patch.py разбирал markdown-таблицу плана по хардкодным индексам ячеек (cells[2]/cells[3]); добавление новой колонки «ТВС» сдвинуло все последующие индексы и молча вернуло уже однажды починенный баг — предупреждение об этом классе риска уже прозвучало в предшествовавшей пир-сессии, но предыдущий фикс исправил только конкретный симптом, не общий класс. | active |
| DP.FM.474 | RLS-политика падает на живом запросе при сравнении text-возврата session-функции с uuid-колонкой без явного каста | RLS-выражение (USING/WITH CHECK) сравнивает text-результат session-helper функции с uuid-колонкой напрямую — Postgres роняет запрос ('operator does not exist: uuid = text'), но только в момент первого реального запроса через эту политику, не при применении DDL. | active |
| DP.FM.475 | После рефакторинга «две ветки → одна общая функция» точки вызова остались с разным набором аргументов — обе получают «certain», но по разным источникам | Две ветки объединили в общую функцию, но точки вызова остались асимметричными: одна передаёт пустую строку вместо owner_session_id, другая — реальное значение из собственного payload. Обе получают «certain», каждая верна по своей локальной логике; объяснён только разный порог confidence, про пропуск owner_session_id комментария нет. | active |
| DP.FM.476 | Потребитель сравнивает severity с одним словарём, а отправитель заранее перевёл значение в другой (red/yellow vs critical/warning) — сравнение всегда ложно, ошибки нет | Скрипт-отправитель уведомлений переводит внутренний словарь severity (red/yellow) в словарь Grafana (critical/warning) до того, как метка попадает в Loki. Шаблон сообщения сравнивал с буквальным «red» — иконка молча неверна; счётчик закреплённого сообщения фильтровал по red/yellow — совпадал с нулём записей. Тесты зелёные; тот же класс повторился в соседнем файле через день, потому что при первом исправлении класс не обобщили. | active |
| DP.FM.477 | Флаг, принимаемый общим парсером CLI, но не читаемый конкретной подкомандой, молча игнорируется | Общий парсер аргументов принимает флаг на любом вызове, а читает его лишь часть подкоманд. `open --isolate --repo X` изолировал репозиторий текущей рабочей директории, а не X: подкоманда `open` флаг `--repo` не читает вовсе. Ни ошибки, ни предупреждения; агент наступил на это дважды подряд в одной сессии. | active |
| DP.FM.478 | Запись в журнал тактов до долговечного коммита артефакта: «готово» без сохранённого результата, а первый fail-fast «фикс» оказался хуже бага | У шага «открыть месяц» три исхода, а журнал знал два: успех, провал и невидимый третий — «файл создан, коммит или публикация не прошли». Эмиссия шла до долговечного коммита; первый «фикс» (fail-fast при неудачной записи) мог оставить несохранённый файл, который проверка идемпотентности «уже готово?» навсегда считала бы результатом. Лечение: эмиссия только после коммита, «ok» только при успешной публикации, иначе «degraded». | active |
| DP.FM.479 | `cp -r` линкованного git-worktree копирует указатель на ОБЩИЙ административный каталог, а не репозиторий — правка remote «в копии» переписывает origin всего репозитория | Одноразовую «копию» для живого прогона сделали файловой копией линкованного worktree. Его `.git` — файл-указатель на общий административный каталог главного репозитория, поэтому «копия» делит с оригиналом remote, HEAD и индекс. `git remote set-url origin` внутри неё переписал origin всего репозитория, включая канонический чекаут и worktree параллельных сессий. | active |
| DP.FM.480 | Инструмент проверки без активного семафора сессии молча уходит в глобальный режим вместо явного отказа; фильтр пути пропускает неполный путь, который падает лишь позже | Два заявленных «ограничения закрытия сессии» оказались ошибками вызова, но за ними — острые дефолты. Без активного семафора инструмент не отказывает и не предупреждает, а сканирует все сессии (session_scope пуст → глобальный скан → чужая грязь). Фильтр пути к репозиторию пропускает неполный путь (голое имя вложенного репо), и тот падает лишь на этапе разрешения каталога. Случай единичный, уверенность низкая. | active |
| DP.FM.481 | Отказ «вы не числитесь наставником потока S1» маскировал несуществующий код потока: сообщение об ошибке прав вместо ошибки разрешения имени | Проверка «есть ли право у X на Y» отвечает «права нет», когда Y не разрешился, и этот ответ неотличим от настоящего отказа в правах. Отказ по `/mentor_stream S1` оказался не про права: кода «S1» в LMS нет (реальные коды несут год и номер набора, например `S1-2026.2-T`). Три диагноза подряд строились на этом сообщении; подсказка бота сама учила несуществующему формату. | active |
| DP.FM.482 | Субъект с двумя ролями даёт дубль пары внутри одного снимка — `ON CONFLICT DO UPDATE` отвергает команду целиком, и синхронизация не срабатывает никогда, молча | Снимок собирается из двух источников ролей (явный список привилегированных + выборка из системы-источника) и пишется одной командой с уникальным ключом «объект+субъект». Первый же человек с обеими ролями даёт дубль пары внутри команды; `ON CONFLICT` защищает от конфликта с уже лежащими строками, но не от дубля внутри одной команды, и Postgres отказывается трогать строку дважды. Справочник молча остаётся пустым. | active |
| DP.FM.483 | Пустой ответ системы-источника трактуется как авторитетное «назначенных нет» и снимает доступ у всех разом, без единой ошибки в логе | Запрос к чужой боевой схеме шёл через три соединения таблиц, поэтому любое переименование на стороне монолита превращало результат в пустой список, а не в ошибку. Семантика «применить снимок» честно отзывала доступ у всех наставников разом. Причина не логировалась вовсе. Лечение: предохранитель по числу или доле отзывов за прогон — превышение порога отменяет прогон целиком. | active |
| DP.FM.484 | Применение миграции извне платформы развёртывания не перезапускает потребителя — первый автопрогон после включения доступа падает и оставляет ложный диагноз «прав нет» | Два независимых включателя — переменная окружения сервиса и объект в схеме базы — управляются из разных мест и в неопределённом порядке. Сервис делает единственную попытку при старте: если объекта в схеме ещё нет, попытка падает, и до следующего срока расписания состояние неотличимо от «синхронизация работает, но никого не нашла». Платформа развёртывания о внешней миграции не знает и перезапуск не инициирует. | active |
| DP.FM.485 | Конвейер синка полагался на побочный эффект «продвинуть HEAD», которого у инструмента-проверки нет по контракту — доставка блокировалась навсегда после первого паблиша из изолированной worktree | Ночной синк вызывал защитный iwe-safe-pull на общей канонической копии и ждал обновлённого HEAD, хотя по контракту инструмент только проверяет копию и HEAD никогда не двигает. После перехода части сессий на публикацию через изолированные worktree копия перестала обгонять origin, и каждый тик видел «источник отстаёт» и молча пропускал доставку. Контракт «только проверка, без мутации» нужно читать буквально, а не по прежде наблюдавшемуся поведению. | active |
| DP.FM.486 | Обработчик состояний перехватывал колбэки с чужим префиксом раньше специализированного роутера — два обработчика молча делили одно пространство callback_data | Общий обработчик состояний в профильном и настроечном состоянии перехватывал все входящие callback_data, включая префикс github_*, раньше специализированного github_router. Одно событие обрабатывалось дважды с расходящимся побочным эффектом, а результат решал порядок регистрации, без явной ошибки. | active |
| DP.FM.487 | Пакетный upsert в одной транзакции с ON_ERROR_STOP=1 — одна некорректная строка откатывает весь батч на каждом тике опроса | Скрипт синхронизации оборачивал весь батч (около 580 строк) в одну транзакцию с ON_ERROR_STOP=1. Одна строка исходных данных (дата начала подписки позже даты окончания) нарушила защитное ограничение целевой таблицы, и ни один подписчик не синхронизировался с первого сбоя, повторяясь каждые 15 минут по расписанию таймера. | active |
| DP.FM.488 | Владение путём ≠ владение репозиторием при нескольких checkout одного git-репо | Проверка владения ресурсом сравнивала буквальный файловый путь, поэтому канонический чекаут и зарегистрированная рабочая копия одного и того же репозитория учитывались как два разных владельца одного пути. Исправление группирует принадлежность по общей Git-базе; одинаковый путь в двух действительно разных репозиториях по-прежнему отклоняется. Случай единичный, уверенность низкая. | active |
| DP.FM.489 | git replace/grafts подделывают проверку ancestry «уже доставлено» без изменения origin | Локальные git replace и legacy grafts меняют видимый граф объектов и заставляют ancestry-проверку принять недоставленный коммит за опубликованный. | active |
| DP.FM.490 | Построчный merge=union молча перемешивает строки соседних многострочных записей append-only журнала | Построчный merge=union возвращает успешное слияние, но разрушает границы многострочных YAML-событий при параллельной записи или cherry-pick разошедшейся истории. | active |
| DP.FM.491 | Trap на освобождение блокировки, зарегистрированный до её захвата, снимает чужую блокировку | Процесс, убитый во время ожидания чужой блокировки, выполняет заранее зарегистрированную очистку и снимает ресурс, которым никогда не владел. | active |
| DP.FM.492 | Решение о выходе данных за периметр проверяет имя способа отправки, а не разрешённый адрес назначения | Декларативно локальный путь становится внешним после переопределения адреса в конфигурации, но проверка продолжает доверять имени функции или переменной. | active |
| DP.FM.493 | TOCTOU через символьную ссылку или именованный канал в записываемой staging-папке перед привилегированным процессом | Менее доверенный писатель подменяет тип записи в staging; root-процесс позже следует имени и получает произвольную запись, выполнение или зависание. | active |
| DP.FM.494 | Несовместимость версий инструмента даёт тихий откат резолвера на старую версию вместо ошибки | Зелёная установка и сборка подтверждают успех операции, но не фактическую версию инструмента: резолвер может молча выбрать старую совместимую версию. | active |
...
| DP.ROLE.100 | Публикатор материалов практикума (Practicum Materials Publisher) | Отслеживает согласованные источники (папка на Маке наставника, согласованный Телеграм-чат), определяет поток/встречу нового материала и публикует его в каталоге хранилища с устойчивой ссылкой. При неоднозначности не публикует наугад — передаёт решение агенту или пилоту. | draft |
| DP.ROLE.101 | Оператор публичного ИИ-помощника МИМ | Держит инвариант Service Clause DP.SC.203, включает/выключает генерацию строки вопросов на сайте МИМ, реагирует на аварийное отключение. Не имеет полномочий менять сами инварианты SC.203 без согласования с Owner Role. | draft |
...
| DP.SC.068 | media-library-publish-pipeline | Наставник получает автоматически опубликованный на Google Диске материал занятия (видео/текст/транскрипт) в папке нужного потока с записью в каталоге, без ручной загрузки в большинстве случаев | draft |
...
| DP.SC.203 | Публичный ИИ-помощник по знаниям МИМ на сайте | Незарегистрированный посетитель сайта МИМ задаёт вопрос в открытую строку и получает ответ на реальных знаниях платформы (или честный отказ) — без регистрации, без хранения текста вопроса. | draft |
...
- Missing `summary`: DP.M.454 (DP.M.454-candidate-criterion-probation-fill-exclude-comment.md)
- Missing `summary`: DP.M.455 (DP.M.455-lock-granularity-per-tool-resource-known-at-admission.md)
- Missing `summary`: DP.M.456 (DP.M.456-companion-derivative-artifacts-sync-same-commit.md)
- Missing `summary`: DP.M.457 (DP.M.457-result-fact-depends-on-table.md)
- Missing `summary`: DP.M.458 (DP.M.458-tariff-downgrade-deferred-transition-period-boundary.md)
- Missing `summary`: DP.M.459 (DP.M.459-grant-source-first-class-field.md)
- Missing `summary`: DP.M.460 (DP.M.460-allowlist-source-states-not-new-code-exclusion.md)
- Missing `summary`: DP.M.461 (DP.M.461-portable-in-place-patch-temp-file-mv-instead-of-sed-i.md)
- Missing `summary`: DP.M.462 (DP.M.462-three-levels-of-done-fact-registry-row-verdict.md)
- Missing `summary`: DP.M.463 (DP.M.463-falsification-criterion-feasibility-check-before-fixing.md)
- Missing `summary`: DP.M.464 (DP.M.464-security-definer-for-non-reader-caller.md)
- Missing `summary`: DP.M.465 (DP.M.465-publication-proof-by-anchor-or-three-way-merge.md)
- Missing `summary`: DP.M.466 (DP.M.466-personal-guide-navigation-map.md)
- Missing `summary`: DP.M.467 (DP.M.467-secured-execution-chain-for-read-only-access.md)
- Missing `summary`: DP.M.468 (DP.M.468-strict-integrity-check-runs-before-partial-proof-loop.md)
- Missing `summary`: DP.M.469 (DP.M.469-resolve-heredoc-conditional-once-in-generator.md)
- Missing `summary`: DP.M.470 (DP.M.470-self-recovery-must-abort-stuck-rebase-not-just-detect.md)
- Missing `summary`: DP.M.471 (DP.M.471-two-channel-terminal-status-proof.md)
- Missing `summary`: DP.M.472 (DP.M.472-durable-pending-until-confirmed-delivery.md)
...
- Missing `summary`: DP.FM.437 (DP.FM.437-third-party-api-contract-assumed-not-asserted-field-name-and-normalize-on-write.md)
- Missing `summary`: DP.FM.438 (DP.FM.438-symmetric-canonicalizer-failure-collapses-comparison-to-false-equality.md)
- Missing `summary`: DP.FM.439 (DP.FM.439-envelope-shape-validated-payload-shape-not-contract-levels-are-independent.md)
- Missing `summary`: DP.FM.442 (DP.FM.442-session-search-path-cached-pool-masks-existing-tables.md)
- Missing `summary`: DP.FM.443 (DP.FM.443-idempotency-key-scope-narrowed-to-fix-unrelated-lock-race.md)
- Missing `summary`: DP.FM.444 (DP.FM.444-frequency-throttle-without-duration-threshold-hides-multiday-outage.md)
- Missing `summary`: DP.FM.445 (DP.FM.445-marp-inline-style-not-visually-applied.md)
...
*Generated by `scripts/generate-map.py` on 2026-09-25*