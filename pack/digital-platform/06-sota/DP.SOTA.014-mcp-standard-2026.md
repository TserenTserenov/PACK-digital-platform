...
4. **Гостевой режим (без входа).** Бесплатно доступен только в Claude.ai и Le Chat.

Для публичного справочного сервера без учётных записей это значит: «работает с любым клиентом» по протоколу не равно «доступно любой аудитории бесплатно и без входа». По данным этого ревью для гостевого режима (пункт 4) строка «Переносимость» не выполняется: «любой клиент» не даёт бесплатного доступа без входа. Для протокола (пункт 1) и для клиентов, которые подключают сторонние MCP-серверы, это наблюдение строку не проверяло: тезис остаётся отдельным утверждением карточки, здесь не подтверждённым и не опровергнутым. Ландшафт меняется поквартально. Живая проверка на реальных площадках возможна только у пилота; до неё пункты 2-4 не считать подтверждёнными за пределами даты 18.09.2026. Основной текст карточки не менялся.
