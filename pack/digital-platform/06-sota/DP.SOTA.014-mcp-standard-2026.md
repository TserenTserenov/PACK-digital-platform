...
## 8. Оспаривается (2026-09-21, РП-170)

Уточнение к разделу 3, строке «Переносимость» («MCP-сервер работает с любым AI-клиентом»). Оспаривается не протокол и не его принятие индустрией, а широта слов «любой клиент». Здесь смешаны четыре разных вопроса, их нужно различать:

1. **Совместимость протокола.** Сервер, соответствующий спецификации, подключается к клиентам с поддержкой MCP. Не оспаривается.
2. **Возможность подключить сторонний сервер в обычном чате.** Зависит от клиента. По состоянию на 18.09.2026 (холодное ревью WP-581, сессия `MC-sessions:2026-09/18/2026-09-18-04-wp581-mcp-fpf-archgate`, файл `review-fable-01.md`) Kimi, DeepSeek, Copilot, Алиса и GigaChat сторонние серверы в обычном чате не подключают.
3. **Тариф.** ChatGPT, Gemini, Grok и Perplexity пускают такие серверы только на платных тарифах.
4. **Гостевой режим (без входа) в потребительском чате.** Бесплатно доступен только в Claude.ai и Le Chat. В клиентах для разработчиков и самохостируемых (по тому же ревью: Cursor, Windsurf, VS Code Copilot, Gemini CLI, Open WebUI, LM Studio и другие) сторонний сервер без авторизации подключается бесплатно, поэтому наблюдение относится именно к потребительским чатам.

Для публичного справочного сервера без учётных записей это значит: «работает с любым клиентом» по протоколу не равно «доступно любой аудитории бесплатно и без входа в потребительских чатах». По данным этого ревью для гостевого режима в потребительском чате (пункт 4) строка «Переносимость» не выполняется. Для протокола (пункт 1) и для клиентов, которые подключают сторонние MCP-серверы, это наблюдение строку не проверяло: тезис остаётся отдельным утверждением карточки, здесь не подтверждённым и не опровергнутым. Ландшафт меняется поквартально. Живая проверка на реальных площадках возможна только у пилота; до неё пункты 2-4 не считать подтверждёнными за пределами даты 18.09.2026. Основной текст карточки не менялся.
