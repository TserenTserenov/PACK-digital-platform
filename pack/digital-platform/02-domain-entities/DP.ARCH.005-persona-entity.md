...
> **Зачем эта секция.** Прежняя формулировка («Персона = декларативный артефакт в Git пользователя») склеивала entity и snapshot. Читатель спецификации делал вывод «Персона = файл в Git» и упирался в противоречие: один файл — много версий; кто entity?

**Три сущности одной фразой:** Носитель (человек вне платформы) ≠ Персона (entity-в-IWE, distributed composition — anchor + declarations + refs) ≠ Декларация Персоны (один Git-снимок на момент T). Полное различение с тестами, таблицей и lifecycle — каноническое место [DP.D.052 §10](../01-domain-contract/DP.D.052-persona-memory-context.md#10-носитель--персона--декларация-персоны-v2-2026-05-31), не дублируется здесь.

> **Сокращено 2026-07-12** (детектор непротиворечивости WP-429 нашёл дословное повторение §10 из DP.D.052, добавленное туда же 2026-05-31). Полный текст — по ссылке выше.
...
Персона — **декларативная модель пользователя**, которую создаёт и владеет сам пользователь. Это то, что человек **говорит о себе**: цели, неудовлетворённости, ценности, роли, предпочтения, ограничения, планы и контекст.
...
| **Цели, неудовлетворённости и намерения** | Желаемые изменения, разрывы между текущим и желаемым состоянием, целевые системы, рабочие продукты, программа развития | `PACK-personal/personal-development/` |
...
Самооценка квалификации МИМ и состояний может входить в декларацию Персоны. Официальная квалификация и вычисленные оценки состояний относятся к Памяти.Derived, а не к Персоне. IWE читает только релевантный текущей работе и разрешённый пользователем срез. Вывод агента не записывается в Персону без предложения и явного принятия пользователем.
