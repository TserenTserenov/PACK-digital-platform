...
> **Конвертируются бонусы, не баллы.** Бонусы = `min(points, Σ(active_days_at_qual_i × cap_i))` — ограниченная сумма, учитывающая историю квалификации пилота. Баллы (earned_total) в конвертацию напрямую не идут.
