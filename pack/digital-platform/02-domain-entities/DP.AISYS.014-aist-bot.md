...
| 13 | Как работает персонализация? | How does personalization work? | персонал, адапт, подстра, adapt, personaliz, customize | Бот адаптирует контент на трёх уровнях. 1) Профиль — заполни имя, профессию, интересы и цели в `/settings`, и бот подстроит примеры, стиль и контент под тебя. 2) Тест систематичности — `/test` определяет состояние (Хаос/Тупик/Поворот) и адаптирует стиль уроков, сложность и темы Ленты (DP.M.004). 3) Цифровой двойник — в будущем персональная траектория обучения на основе полного профиля, прогресса и заметок. | The bot adapts content on three levels. 1) Profile — fill in your name, occupation, interests and goals in `/settings`, and the bot will tailor examples, style and content for you. 2) Systematicity test — `/test` determines your state (Chaos/Deadlock/Turning Point) and adapts lesson style, difficulty and Feed topics (DP.M.004). 3) Digital twin — in the future, a personal learning trajectory based on your full profile, progress and notes. |
