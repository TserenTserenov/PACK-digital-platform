...
При подключении ЦД (Настройки → Подключения → Цифровой двойник) данные из БД бота переливаются в `1_declarative` секцию ЦД. Перелив происходит при первом подключении (с согласия пользователя) и при каждом обновлении профиля в боте.
