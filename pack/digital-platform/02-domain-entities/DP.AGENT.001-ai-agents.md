...
### 3.3. Мета-роли владельца (Platform Levels)

> Мета-роли определяют, **на каком уровне системы** действует агент. Один человек может выступать в разных мета-ролях в разных сессиях. См. [11-platform-levels.md](https://github.com/TserenTserenov/DS-ecosystem-development) для полной модели.
...
**Отличие от ролей §3.2:** Роли §3.2 (R1-R21) описывают **что делает ИИ-агент** внутри платформы. Мета-роли описывают **на каком уровне действует владелец/оператор**.

**IntegrationGate:** При интеграции нового инструмента/агента — определить мета-роль: M2 добавляет на платформу, M3 добавляет в шаблон, M4 настраивает для себя. Каждый уровень имеет свой governance.
