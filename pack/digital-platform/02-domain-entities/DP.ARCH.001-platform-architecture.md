...
type: domain-entity
status: active
summary: "3-слойная архитектура ИТ-платформы с 7 характеристиками (ЭМОГССБ) и 34 принципами: эволюционируемость, масштабируемость, обучаемость, генеративность, скорость, современность, безопасность"
created: 2026-02-07
updated: 2026-07-27
trust:
  F: 4
...
| 2 | **Recovery:** Можно восстановить данные после сбоя? Заданы ли целевое время восстановления (RTO) и допустимая потеря данных (RPO)? | Есть бэкап, есть процедура восстановления, она протестирована живым прогоном; recovery-цели названы числом, не «быстро» (WP-548 Ф1: пробел Reliability из Well-Architected закрывается поэтапно — сначала здесь, поднятие в core — отдельным решением) |
...
### 4.15. Экономичность (доменная, L2) — draft

> **Доменная характеристика (L2, WP-507).** Метод оценки: DP.M.005 §9. В отличие от базовых 7 (L1) — не «сколько платит пользователь», а «во что обходится платформе выполнение своей же работы» (вычисления, вызовы LLM, время агента).

**Определение:** Способность решения выполнять свою функцию за минимально достаточный расход ресурсов (токены LLM, вычисления, время агента). Отдельная ось от Скорости (§4.5): быстрое решение может быть дорогим (Opus на каждый шаг), экономичное — не обязано быть мгновенным.

**Триггер включения:** Решение вводит новый автоматический процесс (агент, конвейер, скрипт с LLM-вызовом) или меняет частоту/объём уже существующих вызовов.

**Чеклист (3 вопроса):**

| # | Вопрос | Что проверяет |
|---|--------|---------------|
| 1 | **Reflex-first:** Можно ли часть задачи решить детерминированным скриптом/правилом вместо вызова LLM? | Формализуемая логика не должна платить за суждение, которое не требуется |
| 2 | **Right-sized model:** Модель подобрана по сложности задачи (Haiku/Sonnet/Opus), а не взята с запасом? | Максимальная модель по умолчанию — антипаттерн |
| 3 | **Бюджет виден:** Есть ли способ увидеть расход (токены/время) до того, как он стал проблемой? | Не post-mortem по счёту, а observable в моменте |

**Индикатор:** Расход (токены/операции) на сопоставимую задачу не растёт без явного решения повысить качество. Целевое: измеримо, кандидат метода измерения — WP-507.
...
| 27 | **Intervention Loop как обязательный контур** | Каждая ИИ-система имеет замкнутый цикл: baseline → measurement → drift detection → attribution → update. Без цикла нельзя отличить улучшение от деградации (DP.FM.005). Реализуется через [DP.SC.037](../08-service-clauses/DP.SC.037-agent-trace.md) (agent trace), [DP.SC.038](../08-service-clauses/DP.SC.038-agent-replay.md) (replay), [DP.SC.123](../08-service-clauses/DP.SC.123-platform-observability.md) (internal observability) — принцип формализует то, что эти SC уже обещают | Н |
| 28 | **Reasoning trace + source attribution** | Рекомендация ИИ сопровождается видимой цепочкой рассуждения и ссылкой на источник (Pack, принцип, данные), не голым выводом. Rationale-слой уже собирается [DP.SC.037](../08-service-clauses/DP.SC.037-agent-trace.md) — принцип обобщает его в архитектурное требование | О(бъясн) |
| 29 | **Отменяемость автоматического действия** | Пользователь может отменить, изменить или отклонить любое автоматическое действие ИИ до или после его совершения — не «принять или уйти». Формализовано [DP.SC.198](../08-service-clauses/DP.SC.198-agent-action-reversibility.md) (draft, 27.07) | К |
| 30 | **Тестируемость как условие эволюции** | Изменение защищено проверкой до мержа: юнит-тест — для детерминированного кода, eval на золотом датасете — для LLM-компонентов (юнит-тест не ловит дрейф качества промпта/поведения). Craft-часть (P1: тест без проверки результата запрещён) уже в [DP.SC.172](../08-service-clauses/DP.SC.172-engineering-code-style.md); eval-слой для LLM-компонентов в SC.172 не покрыт — остаётся кандидатом | Э, Б |
| 31 | **Конвейер непрерывной доставки** | Изменение проходит commit → verify → deploy без ручных шагов; lead time и частота доставки — измеримые метрики, отдельные от Скорости рантайма (§4.5). Прямое совпадение с [DP.SC.171](../08-service-clauses/DP.SC.171-conveyor-development.md) (6 станций, двойной выход, WIP-лимит) — принцип не вводит новое, ссылается на уже существующий контракт | С(кор, доставка) |
| 32 | **Reflex-first** | Детерминированный скрипт/правило предпочтительнее вызова LLM, если задача формализуема; LLM — только там, где требуется суждение. Практика уже реализована в Маршрутизаторе (`executor: script`) и в [DP.SC.145](../08-service-clauses/DP.SC.145-llm-router.md) (active) | Экон, С(кор) |
| 33 | **Право-размерные вычисления** | Модель подбирается по сложности задачи (Haiku=тривиальное, Sonnet=закрытый цикл, Opus=открытый цикл), не берётся с запасом по умолчанию. [DP.SC.145](../08-service-clauses/DP.SC.145-llm-router.md) (active) уже это делает через `model_preference: auto` — принцип фиксирует правило выбора как архитектурное, не только операционное | Экон |
| 34 | **Прогрессирующая автономность с откатом** | Автономность агента растёт по измеримому track record (доля успешно закрытых циклов без вмешательства человека), не выдаётся одним актом; регрессия — автоматическое сужение scope, не молчаливое продолжение. Скелет обещания — [DP.SC.199](../08-service-clauses/DP.SC.199-agent-autonomy-progression.md) (draft, 27.07, механизм не спроектирован — открытые вопросы в тексте SC) | М, Б, К |
...
|---|---|---|
| **Э**волюционируемость | #1, #2, #3, #4, #5, #7, #12, #13, #14, #15, #30 | Сильное (11) |
| **М**асштабируемость | #3, #6, #8, #11, #13, #16, #17, #19, #23, #25, #34 | Сильное (11) |
| **О**бучаемость | #4, #21, #25 | Достаточное (3) |
| **Г**енеративность | #6, #9, #15, #16, #24 | Достаточное (5) |
| **С**корость | #19, #20, #31 | Достаточное (3) |
| **С**овременность | #18, #20, #23 | Достаточное (3) |
| **Б**езопасность | #8, #10, #11, #12, #14, #17, #18, #30, #34 | Сильное (9) |
| **Экон**омичность (L2, WP-507) | #32, #33 | Новое (2). Первые принципы сразу при вводе характеристики (§4.15) |
| **П**ереносимость данных (L2) | #6, #18, #24 | Сильное (3). Отчуждаемость + Open-source first + Data Portability by Default |
| **Н**аблюдаемость (L2) | #27 | Слабое (1) — было 0. Intervention Loop как обязательный контур |
| **О**бъяснимость (L2) | #21, #28 | Достаточное (2) — было 1. Reasoning trace + source attribution формализован |
| **В**оспроизводимость (L2) | #14 | Слабое (1). Event Sourcing помогает, но не гарантирует консистентность оценок |
| **К**онтролируемость (L2) | #21, #29, #34 | Достаточное (3) — было 1. Отменяемость действия + откат автономности |
| **С**охранность знаний (L2) | #14, #10 | Достаточное (2). Event Sourcing + Secrets вне git; recovery покрыт вопросом-чеклистом §4.13 (RTO/RPO, WP-548), не отдельным принципом |
| **И**нтероперабельность (L2) | #2, #7, #15 | Достаточное (3). Слабая связанность + Реестры + Multi-surface |

> **Обнаруженные пробелы, закрытые 27.07.2026 (WP-507):** Наблюдаемость (#27), Объяснимость (#28), Контролируемость (#29) — все три были явными кандидатами в предыдущей версии этой секции; сформулированные там тексты перенесены в принципы без изменений по смыслу.
>
> **Остаются открытыми** (candidates для новых принципов):
> - **Обучаемость** — ✅ закрыт принципом #21 (Экзоскелет) для пользователя. Для платформы (самообучение системы, а не только пользователя) — остаётся: capture-to-pack, feedback loop как принцип, не только практика
> - **Скорость** — #31 закрыл lead time доставки; latency budget per layer / runtime performance SLA всё ещё не формализован
> - **Воспроизводимость** (L2) — #14 недостаточен, нет принципа про version pinning модели/промпта
> - **Сохранность знаний** (L2) — RTO/RPO покрыты вопросом §4.13 (WP-548, 31.08.2026), но остаются пунктом чеклиста, не отдельным принципом уровня 2 — кандидат на будущее оформление, если чеклиста окажется недостаточно (см. WP-507, кандидат-характеристика «время восстановления»)
