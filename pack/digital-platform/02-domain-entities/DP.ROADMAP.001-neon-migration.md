...
- **OwnerIntegrity** — факт пишется только туда, где его владелец (DP.D.050).
...
1. **Монолитный ЦД** — `digital-twin` смешивал декларативные данные пользователя (ответы, настройки), расчётные показатели (baseline, potential) и runtime-контексты агентов. Разный writer — разный владелец (DP.D.050).
...
|---|---|---|
| `neon-migrate.sh` | pg_dump/restore между БД Neon | `DS-ecosystem-development/scripts/neon/` |
