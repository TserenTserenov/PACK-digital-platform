...
### 13.1. Один сценарий за раз

Первое освоение IWE строится не вокруг одновременной настройки всех возможностей, а вокруг **одного повторяемого пользовательского сценария**. Пользователь выбирает реальную работу, проводит её через полный цикл open → work → close, проверяет результат и только затем добавляет следующий сценарий.

Минимальный цикл внедрения:

1. Выбрать одну повторяющуюся работу с понятным внешним результатом.
2. Передать только необходимый для неё контекст.
3. Ограничить действия агента обратимым и наблюдаемым контуром.
4. Принять результат человеком и записать найденные правила.
5. Повторить сценарий до устойчивости; после этого расширять полномочия или добавлять следующий.

**Антипаттерн:** начинать с полной миграции жизни, всех проектов и всех прав доступа. Это смешивает ошибки среды, метода, контекста и доверия, поэтому пользователь не может понять, что именно не сработало.

**Источник применения:** семинар IWE 2026-08-08; участникам рекомендован постепенный вход через один жизненный или рабочий сценарий.
