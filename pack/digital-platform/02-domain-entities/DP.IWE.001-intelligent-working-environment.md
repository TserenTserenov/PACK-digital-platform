...
> **Архитектурная позиция (21 мая 2026, atributed 29 мая 2026):** IWE = **дистрибутив** (слой 4) на основе агентного runtime (слой 1). Аналогия: Ubuntu на ядре Linux. Ядро — **Hermes Agent (Nous Research, <https://hermes-agent.nousresearch.com/>)**, слой 1 — исполняет агентные задачи; дистрибутив (IWE, слой 4) добавляет методологию, Pack-знания, протоколы ОРЗ и персональную конфигурацию. При смене runtime-хоста дистрибутив переезжает без изменения методологии. Подробнее: § 5.2.
...
| 1 | Hermes Agent (Nous Research, базируется команда Андрея) | Команда Андрея (на базе Nous Research) | Агентный runtime (исполнение задач, очереди). Источник: https://hermes-agent.nousresearch.com/ |
| 2 | MCP (Model Context Protocol) | Anthropic | Протокол инструментов |
| 3 | Claude Code / интерфейс-хост | Anthropic / партнёры | Хост-интерфейс для агентов |
...
**Два лагеря в разработке:**

| Лагерь | Слои | Что создаёт |
|--------|------|-------------|
| Команда Андрея | 1–3 | Runtime, MCP-инструменты, Claude Code extensions |
