...
> **Архитектурная позиция (21 мая 2026, исправлено 05.09.2026 — Hermes не единственное ядро):** IWE = **дистрибутив** (слой 4) на основе агентного runtime (слой 1). Аналогия: Ubuntu на ядре Linux — с поправкой, что ядро здесь не одно: слой 1 занимает любой из нескольких независимых рантаймов (Claude Code и claude.ai — Anthropic, Hermes Agent — команда Андрея на базе Nous Research <https://hermes-agent.nousresearch.com/>, Kimi, бот-раннер платформы), а не единственный поставщик. Дистрибутив (IWE, слой 4) добавляет методологию, Pack-знания, протоколы ОРЗ и персональную конфигурацию поверх любого из них. При смене runtime-хоста дистрибутив переезжает без изменения методологии. Подробнее: § 5.2.
...
**Аналогия 2 (Банковское приложение):** Веб-приложение IWE — как банковское приложение для интеллектуального капитала. Банковское приложение показывает финансовое состояние (баланс, транзакции, цели накоплений) — IWE Web App показывает состояние развития через Цифровой двойник (навыки, прогресс, цели развития). Как банк даёт дашборд личных финансов — IWE даёт дашборд личного развития: что освоено, что в процессе, куда двигаться дальше. У пользователя также есть **баланс активных действий** — токены (баллы), начисляемые за выполнение ДЗ, написание постов, помощь в развитии экосистемы. Баланс токенов влияет на статус в программе лояльности — как кешбэк и бонусные мили в банковском приложении. Конкретная раскладка этой аналогии на существующие таблицы/UX-паттерны (что уже готово к экрану, что требует доработки модели данных) — [UX-референс банковского приложения](../../../../DS-ecosystem-development/C.IT-Platform/C2.IT-Platform/C2.2.Architecture/Stack-and-Infrastructure/UX-banking-app-reference.md) (DS-ecosystem-development, РП-5 backlog #7); архитектурная граница любого экрана — `DP.SC.031`/`DP.SC.032`.

### 1.1. Операционный принцип: развитие внутри реальной работы

Человек обращается к IWE, чтобы продвинуть реальный РП, проект или рабочий продукт, а не чтобы отдельно пройти урок. IWE использует минимально необходимый и разрешённый пользователем личный контекст: цели, неудовлетворённости, планы и текущую работу. К этому срезу добавляются квалификация МИМ и независимые оси состояния, если для них есть данные с известным источником и временем наблюдения.

Среда определяет ближайший полезный шаг и предъявляет нужный метод или различение в момент применения. Допустимый выбор — не добавлять развивающее вмешательство, если оно не помогает текущей работе.

Один рабочий цикл публично описывается тремя результатами: изменён внешний проект, развился человек, сохраняемо обновился персональный IWE. Формальная инженерная модель `PD.FORM.131` различает четвёртый наблюдаемый результат — изменение отношения и совместной способности человека с IWE; публичная тройка его не отменяет.

Полный набор шести осей состояния — целевая модель. Неизвестная или ещё не готовая к исполнимому выбору ось остаётся `null`, а не заменяется предположением системы.

### 1.2. Место IWE: агент, модель, платформа (уточнение 2026-09-05)

Разговор о месте IWE иногда ставит модель («LLM») в один ряд с IWE и Aisystant как самостоятельную систему. Это лишний уровень абстракции: у модели нет состояния между сеансами, она не различает пользователей и не хранит историю — состояние сеанса держит агент, а историю работы — IWE.

Правильный уровень описания — четыре элемента разного типа:

| Элемент | Статус | Что это |
|---|---|---|
| **Агент** | рантайм сеанса | связывает модель, IWE и сервисы Aisystant в конкретном сеансе. Бывает платформенный (Telegram-бот, iwe.system-school.ru), пользовательский (Claude Code, Hermes на машине пользователя) или сторонний (Anthropic в claude.ai) |
| **Модель (LLM)** | сменное ядро агента | интеллектуальная мощность без состояния между сеансами; поставляется как сервис (подписка, платформа) или ресурс (локальная модель) |
| **IWE** | система пользователя | хранилища, правила, навыки, системы исполнения, история работы — переживает сеансы, не зависит от агента или модели |
| **Aisystant** | платформа сервисов | методология, база знаний, цифровой двойник, механизм развёртывания — источник компонентов для IWE |

Модель вызывает агент, не обязательно Aisystant. В Telegram-боте и на iwe.system-school.ru агент платформенный, и там Aisystant действительно управляет вызовом модели. В claude.ai и VS Code агент пользовательский или сторонний, модель вызывается напрямую, а Aisystant подключён сбоку как сервис через MCP. Цепочка «модель → Aisystant → IWE» верна не для всех интерфейсов — модель через агента верна для всех (полная таблица интерфейсов → [DP.IWE.004](DP.IWE.004-iwe-interfaces.md)).

**Доставка ≠ владение** (уточнение к DP.D.095). Скилл, скрипт, правило или шаблон, развёрнутый из Aisystant в IWE, становится частью IWE: пользователь управляет экземпляром, может изменить или не принять следующее обновление. Платформа остаётся источником обновлений, не владельцем. Это не относится к сервисам, к которым IWE обращается, но которые не копируются: цифровой двойник, база знаний, MCP как протокол.
...
### 5.2. IWE как дистрибутив: позиция в 8-слойном стеке

> **Ключевой инсайт (стратсессия 21 мая 2026, «IWE = дистрибутив»):** IWE — дистрибутив на основе агентного runtime. Аналогия: Ubuntu на ядре Linux. Ядро исполняет; дистрибутив добавляет методологию, конфигурацию, инструменты.

Агентная экосистема 2026 имеет 8 слоёв:

| Слой | Компонент | Владелец | Роль |
|------|-----------|----------|------|
| 0 | LLM (модель) | Anthropic / другие поставщики | Базовая интеллектуальная мощность — сменное ядро внутри агента, без состояния между сеансами |
| 1 | Агентный runtime | Разный для каждого агента: Anthropic (Claude Code, claude.ai), команда Андрея на базе Nous Research (Hermes Agent, https://hermes-agent.nousresearch.com/), Kimi, платформа (бот) | Исполнение задач, вызов модели, очереди — реализация зависит от конкретного агента, единого владельца слоя нет |
| 2 | MCP (Model Context Protocol) | Anthropic | Протокол инструментов, общий для всех агентов |
| 3 | Хост-интерфейс | Тот же владелец, что у слоя 1 для данного агента (runtime и хост у одного агента идут парой, не независимо) | Точка входа человека в сеанс: терминал, бот, браузер |
...
| 6 | Персональный экземпляр | Пользователь | Настройки, личный Pack, DS-репо |
| 7 | Экосистема | Сообщество | Когорты, peer-агенты, обмен знаниями |

**Кто строит рантайм.** Единого «лагеря runtime» нет — у каждого агента свой владелец (Anthropic — Claude Code и claude.ai; команда Андрея — Hermes Agent; аналогично для Kimi и бота платформы). Агенты независимы друг от друга: методология IWE (слой 4) подключается к любому из них одинаково, через общий протокол MCP (слой 2), а не через один выделенный runtime-слой.

**Почему «дистрибутив», а не «приложение»:** Дистрибутив определяет *стиль работы поверх runtime*, а не решает одну задачу. IWE не просто вызывает агентов — он задаёт контракты (ОРЗ-протокол, Capture-to-Pack, ArchGate), Pack-сущности, роли и методы. При смене runtime-хоста (Claude Code → Hermes → другой) методология IWE (слой 4) остаётся неизменной; меняется только адаптер на границе слоёв 3↔4.

**Инвариант runtime-цикла:** Любая работа в IWE проходит через цикл **open → work → close** (ОРЗ-фрактал). Этот контракт не зависит от хоста. **DP.IWE.011 Runtime Host Contract** специфицирует open/work/close как хост-агностичный интерфейс для любого runtime (Claude Code, Hermes Agent, бот, headless). → DP.SC.046
...
### 8.1. Операционная лестница зрелости

Для объяснения входа в IWE используется операционная лестница: **чат-бот → настроенный помощник → агент → среда**.

| Ступень | Что уже есть | Чего ещё нет |
|---------|--------------|--------------|
| Чат-бот | Ответ на отдельный вопрос | Устойчивого контекста пользователя |
| Настроенный помощник | Инструкции и контекст текущего способа работы | Самостоятельного цикла действий |
| Агент | Инструменты и цикл выполнения задач | Организации из памяти, правил, ролей и рабочих продуктов |
| Среда | Несколько агентов, общая память, методы, правила, продукты и контуры проверки | — |

Эта лестница описывает не техническую непрерывность личности агента из DP.D.289, а **масштаб организации работы**. Агент может быть технически зрелым, но всё ещё оставаться одним исполнителем вне среды.

Накопленный контекст — капитал пользователя: решения, различения, история проверок и связи между рабочими продуктами повышают отдачу следующих сессий. Хранение контекста только в разрозненных чатах похоже на хранение денег под матрасом: ценность существует, но её трудно учитывать, переносить, защищать и заставлять работать. Поэтому персональная память и Pack должны оставаться доступными пользователю независимо от конкретного поставщика модели.

**Наблюдаемое свидетельство:** семинар IWE 2026-08-08 показал, что переход к среде требует нового мастерства пользователя — ставить задачи, ограничивать полномочия, принимать результат и превращать опыт в правила. Улучшение модели само по себе это мастерство не создаёт.

## 9. Предвосхищаемые вопросы

### «Это vendor lock-in на Claude?»

Нет. IWE — **дистрибутив** (слой 4), не привязанный к конкретному runtime (слои 1–3):
...
- При замене Claude Code → Hermes: меняется хост (слой 3), не методология (слой 4)

Текущий риск: протоколы ОРЗ (open/work/close) написаны с расчётом на Claude Code как единственный хост. WP-348 специфицирует runtime-цикл как хост-агностичный контракт (Ф2, июнь–июль 2026). Подробнее: § 5.2.
...
- [DP.IWE.004](DP.IWE.004-iwe-interfaces.md) — интерфейсы IWE и сборка сеанса (§ 1.2 выше — сжатая версия)
...
- [PD.SPEC.001](../../../../PACK-personal/pack/personal-development/02-domain-entities/formalizations/PD.SPEC.001-tailor-context-contract.md) — минимальный контекст для выбора развивающего шага
- [PD.FORM.131](../../../../PACK-personal/pack/personal-development/02-domain-entities/formalizations/PD.FORM.131-cyberhuman.md) — четыре наблюдаемых результата ко-развития человека и IWE
