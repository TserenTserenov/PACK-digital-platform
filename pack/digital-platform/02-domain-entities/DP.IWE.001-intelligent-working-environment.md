...
| Экзокортекс | IWE |
|-------------|-----|
| Память и инструкции (CLAUDE.md + memory/) | 5 видов: системы + описания + роли + методы + рабочие продукты (§ 3) |
| Хранит знания | Применяет знания в работе |
| Усиливает память | Усиливает мышление, планирование, создание |
| Компонент IWE | Целое |

**Экзокортекс** виден из двух видов. Через линзу «Системы» (Вид 1): файловая система CLAUDE.md + memory/ с 4D-границами, lifecycle (Open/Close), владельцем. Через линзу «Описания» (Вид 2): содержимое этих файлов — различения, протоколы, FPF/SPF. Это не два объекта, а два ракурса на один (ISO 42010). Но экзокортекс — лишь одна из сущностей IWE. Он не включает:

- **Другие системы**, которые исполняют работу (Claude Code, бот, MCP, Git)
- **Роли**, которые определяют функциональные обязанности (Стратег, Экстрактор)
- **Методы**, которые описывают «как делать» (ОРЗ, Capture-to-Pack, ArchGate)
- **Рабочие продукты**, которые производятся (DS-strategy, Pack-репо, события ЦД)
