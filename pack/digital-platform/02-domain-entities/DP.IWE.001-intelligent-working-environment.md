...
type: domain-entity
status: draft
summary: "IWE — персональная интегрированная среда для интеллектуальной работы. Описывается через 4 архитектурных вида (ISO 42010): системы (U.System), знания (U.Episteme), роли (U.RoleAssignment), артефакты (U.Work). Позиционирование: почему именно IWE, а не агенты/экзокортекс/FPF по отдельности."
created: 2026-02-23
trust:
  F: 3
  G: domain
...
**Аналогия:** Операционная система для интеллектуальной деятельности. Как macOS объединяет файловую систему, приложения, терминал и UI в единое рабочее пространство — IWE объединяет знания, агентов, методологию и инструменты в единую среду мышления.
...
| Экзокортекс | IWE |
|-------------|-----|
| Память и инструкции (CLAUDE.md + memory/) | 4 вида: системы + знания + роли + артефакты (§ 3) |
| Хранит знания | Применяет знания в работе |
| Усиливает память | Усиливает мышление, планирование, создание |
| Компонент IWE | Целое |

**Экзокортекс** — знания и инструкции внутри IWE (Вид 2: Знания в § 3). Отвечает за то, ЧТО ИИ знает и КАК себя ведёт. Но экзокортекс — один из четырёх видов IWE. Он не включает:
...
|-----|-----------|---------|------|---------|
| **Системы** | U.System | Claude Code, бот, MCP, WakaTime, launchd, Git | Работает / не работает? | Нет исполнения |
| **Знания** | U.Episteme | Экзокортекс, Pack, промпты, методология (FPF/SPF) | Актуально / устарело? | ИИ = stateless, вайб-работа |
...
> **Почему 4 вида, а не плоский список?** Системы запускаются, знания загружаются, роли назначаются, артефакты производятся. Каждый тип имеет свой жизненный цикл, свои тесты и свои владельцы. Смешение типов в одном списке — категориальная ошибка (FPF A.7 Strict Distinction).

> **Методология** — содержание экзокортекса (Вид 2: Знания), не отдельный элемент. Протоколы, различения, FPF/SPF закодированы в файлах `memory/protocol-*.md`, `CLAUDE.md`, промптах агентов.
...
- IWE: полная среда (4 вида: системы, знания, роли, артефакты — § 3), методология в экзокортексе, агентные роли и эволюция.
...
- [DP.EXOCORTEX.001](DP.EXOCORTEX.001-modular-exocortex.md) — знания и инструкции IWE (Вид 2: Знания)
