...
- T2 → T3: пользователь **подключает любой AI-клиент** → ИИ начинает **знать тебя глубоко** (Память.Derived) и **мыслить методологически** (standard CLAUDE.md)
...
| T3: Персонализация | БР (из T2) | **Нет** | Подключить любой AI-клиент (claude.ai / Claude Code / VS Code / Telegram) → наставник вместо эксперта (0 доп. стоимости) |
| T4: Созидание | БР (из T2) | **Нет** | setup.sh → личный экзокортекс, агенты (Claude Code — свой API ключ) |

> **T3 не стоит дороже T2.** Переход T2→T3 бесплатный: пользователь подключает любой AI-клиент (claude.ai / Claude Code / VS Code / Telegram — WP-406 Ф13) → ИИ становится наставником. Стимул — не платить больше, а получить лучше.
