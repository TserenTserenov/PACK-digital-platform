...
| **Биллинг** | Лимит токенов по тарифу. Trial: базовый лимит. Подписка БР: расширенный лимит. Превышение: уведомление + опция докупить |
| **Опционально** | T4 Direct MCP: пользователь может подключить свой API-ключ для неограниченного использования |

**Зачем:** Убирает барьер «нужен свой ключ Anthropic за $20/мес» — пользователь платит одну подписку и получает всё. Для платформы: дополнительный канал дохода (маржа на AI-вызовах) + контроль качества (выбор оптимальной модели для задачи).

**Доступность по тирам:**

| Тир | LLM доступ | Лимит |
|-----|-----------|-------|
| T0/T1 (триал) | ✅ Платформенный | Базовый (достаточно для знакомства) |
| T2 (подписка) | ✅ Платформенный | По тарифу |
| T3 | ✅ Платформенный | По тарифу (+ персонализированные запросы) |
| T4 (Gateway) | ✅ Платформенный | По тарифу (+ агенты) |
| T4 (Direct MCP) | Платформенный или личный | По выбору пользователя |
