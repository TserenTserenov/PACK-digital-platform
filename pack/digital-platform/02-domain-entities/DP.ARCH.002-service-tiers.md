...
> **Implementation Note.** 4-осевая модель тиров (T0-T4 + TM/TA/TD), 5 измерений, прогрессия, две оси онбординга (§2б) — домен. Конкретные сервисы (WakaTime, system-school.ru), UI-реализация (aiogram FSM, ReplyKeyboard), freemium-механика — текущая реализация. Реализация §8 (build_claude_context), §12 (конверсионная воронка), §13 (Progressive UI) → [C2.IT-Platform / DP.ARCH.002-tiers-implementation.md](../../../DS-ecosystem-development/C.IT-Platform/C2.IT-Platform/C2.2.Architecture/System-Implementations/DP.ARCH.002-tiers-implementation.md).
...
/start → T0 ──────→ T1 ──────→ T2 ──────→ T3 ──────→ T4 (IWE)
    ...
    поиск МИМ     продолж.    на всё)     → наставник → со-мыслитель
...
| **~~Trial~~** | ~~Trial для БР?~~ **Решено (WP-210 Ф2a, WP-349 Ф24):** Trial отменён. Модель freemium — T0/T1 бесплатно навсегда. Подписка БР = персонализация |
