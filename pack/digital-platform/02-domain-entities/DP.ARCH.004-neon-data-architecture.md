...
| points.point_transactions | Лог начислений/списаний баллов активности (append-only, FK на qualification_events для расчёта прогресса) | calculate_points (WP-121 Ф2, TBD) | gateway_reader: баланс, Metabase | §7.0.2 #1 «Транзакция баллов» (DP.ECON.001, WP-121) | —, планируется (WP-121 Ф2) | 🆕 Перенести из #2 digital-twin → #1 (решение 19 апр) | WP-121 |
| points.point_rules | Правила начисления баллов по типу активности | Методсовет (через Directus/seed) | calculate_points (на каждой event) | §7.0.2 #1 «Правило начисления баллов» (DP.ECON.001) | —, планируется (WP-121) | 🆕 Создать | WP-121 |
| points.qualification_multipliers | Множитель квалификации (бонус за ступень) | Методсовет (через Directus/seed) | calculate_points (умножение на базовый балл) | §7.0.2 #1 «Множитель квалификации» (DP.ECON.001) | —, планируется (WP-121) | 🆕 Создать | WP-121 |

### #2 digital-twin

| Таблица | Назначение | Writers | Readers | Pack-сущность | DS-реализация | Статус | Источник |
|---------|-----------|---------|---------|---------------|---------------|--------|----------|
| DIGITAL_TWINS | Цифровой двойник: 3 слоя (базовые / вовлечённость / производные). | Бот dt_sync (`aist_bot_writer`): `2_collected` (cron 04:30), Profiler R28: `3_derived` (standalone) | dt-mcp: профиль по API, tailor-mcp: `1_declarative`+`3_derived`, *бот /twin через gateway→dt-mcp* | §7.0.2 #2 «Цифровой двойник» (DP.D.036, DP.ARCH.003, WP-227) | aist_bot_newarchitecture/db/queries/dt_sync.py, DS-MCP/digital-twin-mcp/src/worker-sse.js, DS-MCP/tailor-mcp/src/queries.py | ✅ | `public.digital_twins`, WP-227 |
| LEARNER_CONCEPT_MASTERY | Степень освоения концептов (0.0–1.0). Основа для квалификации "Ученик". Реализация 🔗-связи «Созидатель освоил Концепт» (DP.METHOD.040 §4: связь с атрибутами `mastery_level`, `last_reviewed_at`). | knowledge-mcp: analyze_verbalization (RLS, WP-208 TBD) | knowledge-mcp: learner_progress (RLS по user_id) | §7.0.2 #2 «Освоение концепта» — 🔗 связь «Созидатель ↔ Концепт» (WP-151, WP-208) | DS-MCP/knowledge-mcp/migrations/005-concept-graph.sql, DS-MCP/knowledge-mcp/src/index.ts | ✅ (физ.реализация 🔗) | `concept_graph.learner_concept_mastery`, WP-151 |

### #3 knowledge

| Таблица | Назначение | Writers | Readers | Pack-сущность | DS-реализация | Статус | Источник |
|---------|-----------|---------|---------|---------------|---------------|--------|----------|
| DOCUMENTS | Документы + векторные эмбеддинги для semantic search. | knowledge-mcp ingest + /reindex webhook (RLS) | knowledge-mcp: hybrid search, *бот через gateway→knowledge-mcp* | §7.0.2 #3 «Документ» | DS-MCP/knowledge-mcp/src/index.ts, DS-MCP/knowledge-mcp/migrations/001-hybrid-search.sql | ✅ | `knowledge.documents`, knowledge-mcp |
