...
| 6 | **Обучение** / `learning` | Память.Observed + Derived | learning-service, наставники через Directus, scheduler лент/дайджестов | зачисление в IWE-программу (ЛР/РР/МР) с активным руководством из вертикального каталога, прогресс по руководству, рабочая тетрадь, ответ ученика, оценка, проверка наставника, попытка практики, Q&A, марафон, обратная связь, лента-неделя, лента-сессия; персональное руководство — опциональная надстройка (Git пользователя, проекция через `#1 persona`) | Зачисление / `enrollment`, Прогресс / `course_progress`, Рабочая-тетрадь / `workbook`, ДЗ / `assignment`, Ответ / `answer`, Оценка / `assessment`, Проверка-наставника / `mentor_review`, Попытка / `practice_attempt`, Вопрос-ответ / `qa_pair`, Марафон / `marathon`, Обратная-связь / `feedback`, Лента-неделя / `feed_week`, Лента-сессия / `feed_session` |
