...
Сплошная стрелка = запись, пунктир = чтение (RO). Детали каждого потока — §4.1–4.5.
...
- **Стадия развития** — слабая сущность в контексте двойника (хранится колонкой `stage` в DIGITAL_TWINS); показана отдельно по §4.0.2 (✅ физ.объект — конкретная позиция созидателя на шкале зрелости).
...
- ⚠️ `FINANCE_PAYMENTS_AUDIT_LOG` = лог изменений статуса (WP-237), на ER не узел (только на физ.схеме §7).
...
**BC:** Bot Learning & Community. **Замечание:** многие объекты (публикация, платёж, концепт) здесь — проекции из других БД. В §7 физ.схема разбита на 3 подраздела (6a/6b/6c) — на концептуальной ER оставляем одну диаграмму.
...
- 🔗 «Напоминание» (`REMINDERS`) = связь Пользователь↔Триггер с атрибутами (тип, время); на физ.схеме есть таблица, на концептуальной ER — не узел (реестр §4.0.2 относит к 🔗).
...
**Навигация:** §3 Карта баз — быстрый ориентир. Нужна конкретная таблица → §7 Справочник (Ctrl/Cmd+F по имени таблицы). Нужно понять связи — §4 Диаграмма или §5 кто читает/пишет.
...
**Структура раздела:**
- **4.0.2** — реестр физ.объектов по БД (основа для ERD вверху документа)
...
>
> **Источники:** Pack (01A-bounded-context, DP.D.050, DP.ROLE.001, DP.ECON.001, DP.ARCH.003, DP.D.034), DP.ARCH.004 §7 (таблицы), WP-155 (CP), WP-227 (ЦД), WP-151 (квалификация).
>
...
| Аккаунт созидателя в канале | OAuth-связка созидателя с каналом (токены в шифре) | CHANNEL_ACCOUNTS | 🔗 связь «Созидатель ↔ Канал» с токенами (payment_credentials класс) | WP-155, §2 П6.1 |
...
<details>
<summary><b>5. Кто читает / кто пишет</b></summary>
...
<details>
<summary><b>6. Пояснения</b></summary>
...
<details>
<summary><b>7. Справочник таблиц</b></summary>
...
<details>
<summary><b>8. Архитектурные решения</b></summary>
...
<details open>
<summary><b>9. Сводная таблица РП</b></summary>
