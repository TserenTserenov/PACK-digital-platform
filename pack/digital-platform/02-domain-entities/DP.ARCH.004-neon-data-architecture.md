...
supersedes: "DP.ARCH.004 v2.1 (22 апр 2026) — 12 БД, ревизия Д1-Д12"
...
| 4 | **Подписка** / `subscription` | Память.Observed | subscription-service (создание грантов, продление, отмена, пауза) | грант подписки (О), автосписание — правило повторного платежа по гранту (О); тарифный план см. `#8 reference.tariff` (К); срез истории изменений гранта `grant_history` — журнальная таблица (О), не физ.сущность | Грант / `grant`, Автосписание / `autopay`, История-изменений-гранта / `grant_history` |
...
| 7 | **Знание-платформы** / `knowledge` | Platform-knowledge (проекция) | knowledge-mcp индексатор (эмбеддинги PACK-digital-platform, PACK-MIM, PACK-ecosystem, ZP, FPF, SOTA, Glossary, руководств) | коллекция — namespace по источнику: `platform.digital-platform`, `platform.MIM`, `platform.sota`, `platform.guides.{program_id}.{guide_id}`, … (О); понятие — запись о концепте онтологии (О); связь между понятиями — строка-артефакт отношения (О); документ (О); параграф (О); эмбеддинг (О); версия индекса (О) | Коллекция / `collection`, Понятие / `concept`, Связь / `relation`, Документ / `document`, Параграф / `paragraph`, Эмбеддинг / `embedding`, Версия-индекса / `index_version` |
...
**Инвариант:** справочник обновляется редко и только админом. Все таблицы имеют `valid_from` / `valid_to` для историзации (temporal validity как перпендикулярный атрибут, WP-257). `REVENUE_SHARE_RULE`: сумма `share_percent` по всем активным правилам для одной пары `(guide_id, payment_kind_id)` должна равняться 100 (проверка на уровне админки Directus, не на уровне CHECK-constraint, чтобы не блокировать временные состояния миграции).
...
erDiagram
    ...
    POINTS_EVENT {
        ...
        numeric amount "положительное число"
        numeric delta "подписанное изменение: +amount для grant/transfer_in, -amount для burn/transfer_out/expire"
        text reason "человекочитаемое описание"
        uuid reference_id "ссылка на исходное событие: payment_id, enrollment_id, counterparty_account_id, …"
...
**Инвариант:**
- `POINTS_EVENT` — event-sourcing ledger (только-добавление): никаких UPDATE/DELETE. Корректировки проводятся обратным `type=adjust` событием с отрицательным `delta`, с `reason` и `actor_account_id`.
...
- `EMISSION_MONTHLY_REPORT` — derived-проекция (С): агрегат всей эмиссии баллов за месяц + сравнение с лимитом из `#8 reference.emission_limit`. Строится регулярно (например, 1-го числа месяца). Отчёт сохраняется неизменяемым, чтобы Методсовет видел исторические уровни эмиссии.
