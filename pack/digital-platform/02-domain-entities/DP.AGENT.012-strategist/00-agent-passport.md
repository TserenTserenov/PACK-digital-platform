...
Claude НЕ начинает нетривиальную работу (>15 мин), если задача не записана как РП в плане текущей недели.

### 7.2. Алгоритм проверки

1. Пользователь даёт задачу
2. Claude проверяет список РП в `MEMORY.md` → «РП текущей недели» (уже в контексте, 0 токенов)
3. **Если есть** → работаем
4. **Если нет** → СТОП:
   - Сообщить: «Этой задачи нет в плане на неделю»
   - Спросить контекст (месячный приоритет, название РП, бюджет)
   - Записать в MEMORY.md и weekly-plan.md
   - Только после этого работать

### 7.3. Исключения

- Задачи ≤15 мин
- Вопросы/исследования без изменений в коде
- Экстренные баг-фиксы в production

### 7.4. Синхронизация MEMORY.md

| Триггер | Действие |
|---------|----------|
| strategy-session (Пн) | Полная перезапись списка РП |
| day-plan (ежедневно) | Обновление статусов (done, in-progress) |
| Новый РП (середина недели) | Добавление в список |
