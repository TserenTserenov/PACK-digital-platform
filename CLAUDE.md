...
## 6. Различения (01B) — режим «индекс+карточка» (с 18.07.2026)

> **Блокирующее (гард `SPF/scripts/check-id-collision-delta.sh`).** `01B-distinctions.md` пересёк порог масштабируемости (SPF.SPEC.003) — новое различение создаётся отдельным файлом `DP.D.NNN-slug.md` в `02-domain-entities/`, НЕ новой секцией в 01B. 01B хранит существующие 133 секции как есть (правки разрешены, новые секции — нет) + служит индексом. Причина, 14 старых пар «файл ≠ секция» с разными номерами (не трогать без отдельного РП — риск ссылок) → `DS-my-strategy/inbox/bugs/drift-2026-07-18-dpd-numbering-collisions.md`.

## 7. Правило синхронизации: MAP.002 ↔ 04-work-products/
...
## 8. Нумерация сценариев (08-service-clauses/)
