...
| **3 слоя работы с ИИ** | Three Layers of AI Work | Разовый запрос (нет контекста) → роль и инструкция (постоянный системный контекст) → накопленная среда (история решений, документы). Переход определяется объёмом контекста, а не моделью | U.Episteme | DP.CONCEPT.004 |
...
| **Сервис (как ситуация)** | Service (as situation) | Пять компонентов: обещание провайдера + провайдерская роль + метод оказания + работа оказания + свидетельства результата. Сервис ≠ метод: метод — лишь один из пяти компонентов. Полисемия: в MAP.002 «сервис» = технический модуль (accessSpec), в FPF A.6.8 = ситуация оказания. DP.D.039 разграничивает оба смысла | U.ServiceClause + U.RoleAssignment + U.Method + U.Work | DP.SC.*, DP.D.039, FPF A.6.8 |
